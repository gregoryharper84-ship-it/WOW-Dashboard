...
# v3 Controlling Patch Integration — WNBA Composite, MLB Directional, and Cross-Ticket Governance

This section supersedes earlier workflow or ceiling language when it is more restrictive.

## WNBA Component and Composite Routing
...
The optimizer must compare every verified current-board stat family for the same player. PRA is not the default.
...
WNBA_forward_test_status
```

Preserve the WNBA skill's lower ceiling.

---

## MLB Directional Firewall Routing

For every MLB pitcher prop, invoke:

```text
wow.mlb-pitcher-failure-path-expert v2
```
...
The optimizer may not upgrade these lanes beyond the active firewall.

---

## Cross-Ticket Exposure Governance

When more than one card is proposed, reviewed, or rebuilt in the same slate, invoke:

```text
wow.cross-ticket-exposure-governor
```

after card-level fragility and before final presentation.

Required slate-level fields:
...
same player-game alternate thresholds
=> one underlying thesis

same pitcher directional thesis on multiple cards
=> keep once

Power card copied from Flex card
=> REJECT_DUPLICATE_STRUCTURE

no independent replacement
=> shrink card
```

Preserving the requested number of cards is not an objective.

---

## Unique-Observation Calibration

Postmortem accuracy must distinguish:

```text
financial_exposure_rows
unique_underlying_thesis_rows
```

Repeated exact legs and alternate thresholds:

```text
count separately for financial exposure
count once for model calibration
```
...
model hit=false
excluded from graded hit/miss numerator unless rules require a graded result
```
...
→ role-matched ledger
→ sport-specific model
→ WNBA joint composite expert when applicable
→ MLB failure-path v2 and directional firewall when applicable
→ market sanity
→ bidirectional score
→ modification search
→ weakest-leg elimination
→ card fragility audit
→ slip rebuild
→ cross-ticket exposure governor
→ lowest-ceiling QA
→ settled-ledger write after grading
```

---

## v3 Added Decision Labels
...
duplicates_removed:
alternate_threshold_groups:
critical_cross_card_thesis:
cross_card_fragility:
WNBA_forward_test_status:
MLB_directional_forward_test_status:
lowest_ceiling:
can_execute=false
```
...
1. WNBA PRA is compared against all verified components.
2. Three alternate Morrow PRA thresholds count once in calibration.
3. A DNP is not a model hit.
4. K LESS remains WATCH_ONLY until the forward test passes.
5. Outs MORE uses unconditional workload survival.
6. Boyd and Rodriguez exact duplicates across Flex and Power are retained once.
7. Copied Power/Flex structures are rejected.
8. No replacement means the card shrinks.
9. Financial exposure and model calibration counts are separated.
10. `can_execute=false` remains enforced.
