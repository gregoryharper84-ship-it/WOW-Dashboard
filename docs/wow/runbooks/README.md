...
- **Production database restoration** is separate from code rollback. A code rollback via Replit checkpoint does NOT restore the production database. Database migrations need their own recovery plan.
...
- **Keepalive daemon:** hits `/wow/engine/health` every 10 min to prevent autoscale SIGTERM at 15-min idle. Requires `REPLIT_APP_URL` environment variable.
