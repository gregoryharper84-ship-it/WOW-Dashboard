...
Render owns the active runtime path for:

- backend governance routes
- sport-model/scorer routes
- health/host-contract routes
- probability-package validation
- event governor
- `V17_TERMINAL_REDUCER` handoff

## Supabase responsibility

Supabase/Postgres owns persistence/reconciliation for:

- calibration ledger
- session ledger
- scored-row persistence
- settlement state
- exact-once/reconciliation records
...
The main WOW Betting Engine must apply this routing rule for team/event probability requests:
...
Do not collapse model invocation, input, or output failures into MODEL_UNAVAILABLE. Preserve:
- MODEL_UNAVAILABLE
- MODEL_INPUTS_INSUFFICIENT
- MODEL_SCORER_FAILED
- MODEL_OUTPUT_INVALID
...
This addendum is authoritative over any older host wording that routes team/event sporting probability through a price-first runner or names Replit as an active acquisition/runtime source.

## Regression requirement

Before trusting any live team/event probability run after instruction, schema, backend, adapter, or patch changes, run the 2026-09-01 model-completion failure regression cases and verify:

- exact `MODEL_UNAVAILABLE`
- exact `MODEL_INPUTS_INSUFFICIENT`
- exact `MODEL_SCORER_FAILED`
- exact `MODEL_OUTPUT_INVALID`
- `rank_eligible=false` on incomplete audit chains
