...
`hit_probability` field with a `calibration_note` string — that let a
haircut/adjustment live only in free text while the number itself
stayed uncalibrated, which risks a downstream consumer reading the raw
figure as the engine's final answer. Corrected contract:

```json
{
  "leg_id": "uuid",
  "raw_model_probability": 0.71,
  "calibrated_probability": 0.68,
  "calibrated_lower_bound": 0.63,
  "hit_probability": 0.68,
  "opposite_raw_probability": 0.29,
  "model_used": "mlb_formula_v2 | claude_poisson_estimate | claude_logistic_estimate | fantasy_score_gaussian_provisional",
  "calibration_note": "string, e.g. small sample size, L5 only"
}
```

`hit_probability` is always the calibrated value — that's the one any
downstream consumer (explanation writer, gate, Custom GPT, human) should
read by default. `raw_model_probability` and `opposite_raw_probability`
should sum to ~1 at the raw distribution layer (MORE/LESS are
complementary there). `calibrated_lower_bound` does not need to sum to
anything with its opposite side — it's a conservative, uncertainty-
adjusted estimate, not a probability partition.
...
**`verified=true` needs to mean verified against settlement, not just
internally consistent.** A first-pass formula (`GAMES_WON×1.0 +
ACES×0.5 + DOUBLE_FAULTS×−0.5`) can be self-consistent with whatever
source it came from while still missing real scoring components.
Before marking Tennis `verified=true`, confirm against the official
source whether the formula also needs: sets won, match win bonus,
straight-set bonus, games lost, tiebreak handling, retirement handling,
and walkovers — and whether best-of-3 vs. best-of-5 changes any
weighting. If only the three coefficients above are actually confirmed,
split the status rather than collapsing it to one boolean:

```
verified_formula = true       (the additive scoring components are confirmed)
verified_settlement = false   (edge cases — retirement, walkover, tiebreak — are not)
```

A verified formula still isn't enough on its own to publish a
probability — role/entry status, match format, and a reconstructed
game-level history are all still required inputs. Formula verification
closes one gap from the original NO_PLAY case, not the whole thing.
...
## Task #84 — Fantasy Score Prop Support (MLB, NBA, WNBA, NFL)

Different shape of work than the sport-expansion tasks above — this is
a **new prop type on sports the pipeline already covers (or will,
after #82)**, not a new sport. No new data source is needed: every
component stat a Fantasy Score formula uses is already being pulled by
the existing game log fetchers. The new work is the formula itself,
a derivation step, and a probability model that handles a weighted sum
instead of a single counted stat.

### Reconstructed formulas

Sourced from PrizePicks' own playbook articles plus cross-referenced
calculator sites — treat as high-confidence starting points, not
final. **Verify against the official PrizePicks playbook pages and a
handful of settled results before trusting any of these at a real
confidence tier** — this is the single highest-risk part of the task,
because a slightly wrong weight doesn't throw an error, it just quietly
produces a wrong L5/L10 hit-rate history that looks plausible.

**NBA / WNBA:**
```
FS = PTS×1.0 + REB×1.2 + AST×1.5 + STL×3.0 + BLK×3.0 + TOV×(−1.0)
```
Confirm WNBA uses the same weights as NBA rather than a separate chart.

**NFL:**
```
FS = (PassYds/25) + (PassTD×4) + (INT×−2) + (RushYds/10) + (RushTD×6)
   + (RecYds/10) + (RecTD×6) + (Receptions×R) + (FumblesLost×−2)
```
Sources disagree on `R` — one figure says 1 point/reception, another
says PrizePicks uses half-PPR (0.5/reception). **Confirm the reception
weight directly from PrizePicks' NFL playbook page before shipping** —
this single value materially changes projections for pass-catchers.

**MLB — Hitter FS:**
```
FS = 1B×3 + 2B×5 + 3B×8 + HR×10 + R×2 + RBI×2 + BB×2 + HBP×2 + SB×5
```

**MLB — Pitcher FS:**
```
FS = Wins×6 + QualityStarts×4 + K×3 + Outs×1 + EarnedRuns×−3
```
Quality start is itself a derived flag (6+ IP, ≤3 ER) — needs to be
computed from the pitcher's own game log, not pulled as a raw field.

### Derivation step (new)

The game log fetchers return raw per-game box scores, not Fantasy
Score directly. Add a pure per-sport function that applies the formula
above to each game in the L5/L10 window, producing the `raw_vals`
series `wow-gate-enforcer`'s Gate 3 already expects. This is
straightforward, sport-isolated work — no changes needed to the gates
themselves once the series exists.

### Probability model

A Fantasy Score is a weighted sum of several differently-shaped
component distributions (points ~Poisson, steals/blocks ~low-rate
Poisson, turnovers ~Poisson with a negative weight) — summing weighted
Poissons does not stay Poisson, especially once a negative weight is
involved. Two viable approaches, same pattern already used for Tennis's
composite in Task #83:

- **Monte Carlo simulation** — sample each component from its own
  fitted distribution many times, apply the formula, build an empirical
  distribution, read off `P(FS ≥ line)`. More accurate, more compute.
- **Gaussian approximation** — mean = weighted sum of component means,
  variance = weighted sum of component variances. Cheaper, but this
  assumes component independence, which isn't quite true (a player's
  points and rebounds and assists all move together with usage/minutes
  in a given game) — flag this explicitly as an approximation that
  likely **understates true variance**, meaning probabilities could
  read more confident than they should. Validate against settled
  results before trusting the calibration.

### Gate placement

No new gate category needed. Fantasy Score slots into the existing
`COMBINED` tier in `wow-gate-enforcer`'s Gate 2 hierarchy (same tier as
PRA, R+A, H+R+RBI) — full confidence once the stat_key is recognized
and the L5/L10 series exists.

### Relative sizing

Lower lift than Tennis or NFL sport-expansion — no new data source, no
new gate logic, no new sport onboarding. The real work is: get the
formulas exactly right (verification-heavy, not build-heavy), write
the derivation function per sport, and pick + validate a probability
approach for weighted composites. Good candidate to build right after
NFL support, since NFL Fantasy Score depends on NFL game logs existing
first.

### Recommended approach (confirmed: no free API provides Fantasy Score directly)

PrizePicks has no public API, and its own official settlement runs
through paid enterprise data providers (SportRadar, Genius Sports,
Stats Perform, Grid) — not available at this scale for free. Computing
it in-house from already-fetched component stats, as scoped above, is
the only free-tier path, which confirms the plan rather than changing
it. Four concrete recommendations to execute it well:

**1. Build order: NBA/WNBA → MLB → NFL.** NBA/WNBA has one universal
formula and the most mature game-log source already. MLB has two
formulas (hitter/pitcher) but a very mature data source (MLB Stats
API). NFL goes last since it depends on Task #82 landing first and
still has an unresolved reception-weight ambiguity to confirm.

**2. Start with the Gaussian approximation, not Monte Carlo.** Ship
the cheaper method first — mean/variance from the component stats,
assuming independence. Only invest in full Monte Carlo simulation if
calibration review (`wow-clv-grader` / `wow-llp-calibration`) shows the
independence assumption is actually costing real calibration accuracy.
Don't build the expensive version speculatively; validate the cheap one
first and upgrade only if the data says to.

**3. Validation loop: reuse existing infrastructure, don't build new.**
Every settled slip that included a Fantasy Score leg is already a
calibration data point — run it through `wow-postmortem-auto` and
`wow-clv-grader` like any other leg. No new tracking system needed;
this is exactly what those skills already do, just applied to a new
stat type.

**Delivery status: built.** `gate_engine/fantasy_score.py` shipped with
all four formulas, 68 passing tests, and — beyond what was asked for —
machine-readable `FORMULA_FLAGS` (e.g. `NFL_RECEPTION_WEIGHT_UNCONFIRMED`,
`WNBA_WEIGHTS_ASSUMED_SAME_AS_NBA`) enforced by test assertions, so the
open questions can't be silently lost. All six new model registry
entries are `PROVISIONAL` / `money_grade_allowed=False` until back-tested
against settled results. A real routing bug (Fantasy Score matching the
counting-stat keyword heuristic and getting misrouted to Poisson instead
of the Gaussian composite path) was caught and fixed in their own testing
before shipping — a good sign the test discipline is doing its job.

**Open follow-up: Rule 5 isn't wired into the automated pipeline yet.**
The correlation-guard update (same-player component/composite overlap)
lives in the `wow-correlation-guard` skill, which only fires when
explicitly invoked in chat or by the GPT. If `/analyze-and-score`
computes its own `correlation_risk` field programmatically rather than
by invoking that skill, it needs the same same-player check added at
...
**4. New correlation-guard rule needed: component/composite overlap.**
`wow-correlation-guard` doesn't currently have a category for this: a
slip containing both "Player Points Over X" and the same player's
"Fantasy Score Over Y" isn't two independent legs — Fantasy Score
*contains* points as a weighted component, so both legs move together
on the same underlying performance. Add a same-player
component-overlap check to correlation-guard: any Fantasy Score leg
paired with an individual-stat leg on the same player in the same slip
should flag `SAME-PLAYER COMPONENT OVERLAP`, the same way same-game
stacking is flagged today. This is a real gap the current correlation
categories (same-game, same-team, script-dependent, pitcher-matchup)
don't cover, and it becomes relevant the moment Fantasy Score ships.

**Delivery update — confirmed shipped:** the composite/component
overlap check landed in code, producing reusable `shared_thesis_keys`
rather than requiring the skill to be manually invoked. This closes
the follow-up gap noted above — worth confirming `shared_thesis_keys`
is what `wow-correlation-guard`'s Rule 5 output maps to, so the skill
and the code stay describing the same mechanism rather than drifting
into two names for the same thing.

**5. Formula registry needs version/hash-based reload, not just
worker-restart.** Reloading the formula registry only on worker
restart is acceptable for a first release, but it's operationally
fragile — a formula edited on disk could keep being served by a
long-running worker that hasn't restarted. Add
`formula_registry_hash` / `formula_registry_version` /
`formula_loaded_at`, reload when the file hash or mtime changes (or
include the hash in the governance handshake), and **persist the exact
formula version/hash on every prediction record, not just the current
registry state.** This isn't just an operational nicety — it's required
for the calibration loop to work at all. If a formula gets corrected
later, back-testing against settled results needs to know which
formula version actually produced each historical prediction, or a
correction retroactively corrupts the ability to evaluate past
predictions.
