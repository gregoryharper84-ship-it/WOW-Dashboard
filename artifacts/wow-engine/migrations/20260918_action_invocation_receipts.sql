...
create table if not exists public.wow_action_invocation_receipts (
    ...
    -- Which class of host called. This is what distinguishes a live Custom GPT
    -- invocation from CI/synthetic traffic, which the canary could never show.
    caller_class text not null default 'UNKNOWN'
        check (caller_class in ('CHATGPT_ACTION','GITHUB_ACTIONS','RENDER_INTERNAL','SELF_ACCEPTANCE','OTHER','UNKNOWN')),
    ...
    constraint wow_action_invocation_route_ck
        check (route in ('/score-pick-request','/score-team-event-request','/score-team-event'))
);
