-- WOW v16 Clean Core — NFL full-game moneyline fitted-specialist P0.
--
-- P0 creates the governed artifact/capability/routing boundaries only. It
-- intentionally seeds NO fitted model and NO calibrator. NFL probability stays
-- UNAVAILABLE until a later evidence-backed certification migration promotes a
-- real artifact. Market or caller probabilities may not substitute.
...
create table if not exists public.wow_nfl_event_fitted_model_artifacts (
    ...
    constraint wow_nfl_event_fitted_provider_identity check (
        provider_identity = 'WOW_NFL_EVENT_FITTED_MODEL_V1'
    ),
    constraint wow_nfl_event_fitted_lifecycle check (
        lifecycle_state in ('CANDIDATE','PROSPECTIVE_CERTIFIED','CHAMPION','RETIRED','BLOCKED')
    ),
    constraint wow_nfl_event_fitted_training_rows_positive check (training_rows > 0),
    constraint wow_nfl_event_fitted_payload_object check (jsonb_typeof(artifact_payload) = 'object'),
    constraint wow_nfl_event_fitted_validation_object check (jsonb_typeof(validation_metrics) = 'object'),
    constraint wow_nfl_event_fitted_checksum_shape check (artifact_checksum ~ '^[0-9a-f]{64}$'),
    constraint wow_nfl_event_fitted_bundle_shape check (bundle_fingerprint ~ '^[0-9a-f]{64}$'),
    constraint wow_nfl_event_fitted_code_sha_shape check (training_code_sha ~ '^[0-9a-f]{40,64}$'),
    constraint wow_nfl_event_fitted_dataset_hash_shape check (training_dataset_hash ~ '^[0-9a-f]{64}$'),
    constraint wow_nfl_event_fitted_never_execute check (can_execute = false),
    constraint wow_nfl_event_fitted_registry_not_probability_publication check (probability_publishable = false),
    constraint wow_nfl_event_fitted_certified_requirements check (
...
create or replace function public.wow_nfl_event_certified_model_artifact(
...
begin
    ...
    if not found then
        return jsonb_build_object(
            'ok', false,
            'code', 'NFL_EVENT_CERTIFIED_MODEL_ARTIFACT_NOT_FOUND',
            'provider_identity', 'WOW_NFL_EVENT_FITTED_MODEL_V1',
    ...
    return jsonb_build_object(
        'ok', true,
        'code', 'NFL_EVENT_CERTIFIED_MODEL_ARTIFACT_READY',
...
revoke all on function public.wow_nfl_event_certified_model_artifact(text)
from anon, authenticated;

-- The controlling specialist route is canonical even while its fitted artifact
-- is unavailable. Missing artifact therefore terminates as MODEL_UNAVAILABLE;
-- no generic/qualitative specialist may replace it.
insert into public.wow_specialist_registry
    (sport, market_family, controlling_specialist, active, precedence, can_execute)
values
    ('NFL', 'OUTRIGHT_WINNER', 'wow.nfl-game-win-probability-expert', true, 100, false)
...
-- Register the capability fail-closed. A later certification migration may
-- change this to AVAILABLE only after a real fitted artifact + calibrator pass
-- the applicable calibration-health and prospective-validation gates.
insert into public.wow_runtime_capabilities
    (capability_key, capability_status, evidence, can_execute)
values (
    'NFL_EVENT_PROBABILITY',
    'UNAVAILABLE',
    jsonb_build_object(
        'provider_identity', 'WOW_NFL_EVENT_FITTED_MODEL_V1',
        'controlling_specialist', 'wow.nfl-game-win-probability-expert',
        'feature_schema_version', 'NFL_EVENT_FEATURES_V1',
        'p0_registry_ready', true,
...
begin
    a := public.wow_nfl_event_certified_model_artifact('NFL_EVENT_FEATURES_V1');
    select * into c from public.wow_runtime_capabilities where capability_key='NFL_EVENT_PROBABILITY';
    select * into s from public.wow_specialist_registry where sport='NFL' and market_family='OUTRIGHT_WINNER';
    ...
    return jsonb_build_object(
        ...
        'specialist_route_ready', found and coalesce(s.active,false),
        'artifact_state', a,
        'capability_status', coalesce(c.capability_status,'UNAVAILABLE'),
        'model_status', case
            when coalesce((a->>'ok')::boolean,false) and coalesce(c.capability_status,'UNAVAILABLE')='AVAILABLE'
              then 'AVAILABLE'
...
revoke all on function public.wow_nfl_event_p0_readiness()
from anon, authenticated;
