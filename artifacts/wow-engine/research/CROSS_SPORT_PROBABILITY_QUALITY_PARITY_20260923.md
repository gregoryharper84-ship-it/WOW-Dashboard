...
The targeted basketball maintenance workflow passed on the acquisition/maintenance repair. The strict WNBA/NHL quantitative-calibration change has its own bridge/typed-failure regressions committed and must pass the full `wow-verify` + `wow-engine-verify` PR checks before the Class B slice can be considered review-ready. Because PR #783 changes a publication prerequisite for WNBA/NHL, it remains draft and is not an automatic merge candidate.
