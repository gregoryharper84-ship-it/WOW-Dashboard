...
class GovernedProbabilityEnvelope:
    ...
    def validate(self) -> None:
        ...
        if self.calibrated_lower_bound is not None and self.calibrated_upper_bound is not None:
            if self.calibrated_lower_bound > self.calibrated_upper_bound:
...
class CertifiedNumericalAdapter:
    ...
    def key(self) -> tuple[str, str, str, str]:
        return (
            self.lane.value,
            self.sport.strip().upper(),
            self.market_or_stat.strip().lower(),
            self.controlling_specialist.strip(),
        )
...
class CertifiedNumericalRegistry:
    ...
    def resolve(self, request: CertifiedComputationRequest) -> CertifiedNumericalAdapter | None:
        ...
        candidates = (
            (request.lane.value, sport, market, specialist),
            (request.lane.value, sport, "*", specialist),
            (request.lane.value, "*", market, specialist),
            (request.lane.value, "*", "*", specialist),
        )
...
def execute_certified_computation(
    ...
    *,
    ...
    if adapter is None:
        return NumericalExecutionOutcome(
            False,
            failure=NumericalFailure.CERTIFIED_ADAPTER_UNAVAILABLE,
            detail="no_exact_specialist_adapter",
        )
    ...
    try:
        ...
        result = replace(
            ...
            computation_version=adapter.computation_version,
            feature_vector_version=request.feature_vector_version,
        )
    ...
    except Exception as exc:  # typed scorer failure; never MODEL_UNAVAILABLE
    ...
    if request.verification_required:
        if adapter.verifier is None:
            return NumericalExecutionOutcome(
                False,
                failure=NumericalFailure.COMPUTATION_VERIFICATION_FAILED,
                detail="required_verifier_unavailable",
            )
        try:
            verifier_probability, verifier_method = adapter.verifier(request, result)
            status, delta = verify_independent_probability(
                primary_probability=result.unconditional_probability,
                verifier_probability=float(verifier_probability),
                tolerance=request.verification_tolerance,
            )
        ...
        result = replace(
            ...
            verification_method=verifier_method,
            verification_probability=float(verifier_probability),
            verification_delta=delta,
            verification_tolerance=request.verification_tolerance,
        )
        if status is VerificationStatus.CONFLICT:
            return NumericalExecutionOutcome(
                False,
                failure=NumericalFailure.COMPUTATION_VERIFICATION_CONFLICT,
                detail=f"delta={delta:.12g}",
            )
