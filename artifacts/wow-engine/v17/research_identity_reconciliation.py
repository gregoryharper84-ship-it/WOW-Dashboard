...
No fuzzy auto-linking is allowed here. A provider row may attach to a candidate
only through a verified provider_entity_map entry or a caller-supplied exact
canonical identifier. Ambiguity remains visible and research-only.
"""
...
def resolve_provider_entity(provider: str, sport_key: str, entity_type: str, provider_entity_id: str | int | None) -> IdentityResolution:
    ...
    with psycopg.connect(database_url()) as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                select canonical_entity_id, canonical_name, verified
                from wow_scout.provider_entity_map
                where provider=%s and sport_key=%s and entity_type=%s and provider_entity_id=%s
                """,
                (provider, sport_key, entity_type, pid),
            )
            rows = cur.fetchall()
    if not rows:
...
def link_candidate_snapshot(candidate_id: str, snapshot_id: str, resolution: IdentityResolution, *, reason: str | None = None) -> dict[str, Any]:
    ...
    entities = {
        ...
        "sport_key": resolution.sport_key,
        "entity_type": resolution.entity_type,
        "provider_entity_id": resolution.provider_entity_id,
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                on conflict (candidate_id,snapshot_id) do update set
                  ...
                  linked_at=now(), prediction_authority=false, can_execute=false
                """,
                (candidate_id, snapshot_id, status, reason or resolution.reason_code, __import__("json").dumps(entities)),
            )
        conn.commit()
