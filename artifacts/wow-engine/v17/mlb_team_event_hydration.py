...
def _caller_fallback(req: Any, *, blocker_code: str) -> dict[str, Any] | None:
    """Build a typed fallback only from complete, explicit MLB model inputs.

    The fallback is intentionally narrow: venue + both starters must be present,
    source_snapshot_id must be explicit, and latest_material_update_timestamp must
    be a valid aware timestamp no later than now. This restores scorer ingress
    when canonical persistence is unavailable without granting caller evidence
    authority over an existing contradictory canonical row.
    """
...
def resolve_mlb_team_event_evidence(req: Any, *, event_api: Any) -> dict[str, Any]:
    ...
    snapshot at or before request time is used. Event participants and start time
    are cross-checked against the caller contract; mismatches fail closed.
    """
