...
def _prop_rows(db: Any, requested_date: str, requested_timezone: str, limit: int) -> list[dict[str, Any]]:
    ...
    return [
        ...
        if not row.get("blockers") and _future(row.get("event_start_time")) and matching_local_date(row)
    ][:limit]
...
def run_daily_snapshot(req: DailySnapshotRequest, *, db: Any, market_api: Any, event_api: Any) -> dict[str, Any]:
    try:
        ...
        return {
            ...
            "terminal": True,
            "run_status": "RUN_INVALID_REQUEST",
            "rows": [],
            "blockers": ["REQUESTED_SLATE_DATE_INVALID"],
            "can_execute": False,
        }
    ...
    if "PROPS" in requested_lanes:
        ...
        for row in prop_rows:
            ...
            rows.append(_terminal_row(
                ...
                {
                    "outcomes": outcomes,
                    "probability_publishable": any(bool(x["payload"].get("probability_publishable")) for x in outcomes),
                    "can_execute": False,
                },
    ...
    if "MONEYLINE" in requested_lanes:
        ...
        for event in event_rows:
            ...
            request = TeamEventRequest(
                ...
                event_key=f"MLB:{event['official_event_id']}",
                official_event_id=str(event["official_event_id"]),
                event_start_time_utc=event["event_start_time"],
                sport="MLB",
                league="MLB",
                ...
                sport_specific_evidence={
                    ...
                    "home_starting_pitcher": event.get("home_probable_pitcher"),
                    "away_starting_pitcher": event.get("away_probable_pitcher"),
            ...
            try:
                ...
                result = {
                    "code": "TEAM_EVENT_SCORER_EXCEPTION",
                    "error_type": type(exc).__name__,
                    "probability_publishable": False,
                    "can_execute": False,
                }
...
def install_daily_snapshot_route(app: FastAPI, *, auth_dependency: Any, db_client_fn: Any, market_api: Any, event_api: Any) -> None:
    # The forward-cohort route shares the same V17 activation boundary and the
    # same production auth/DB/scorer dependencies. Mount it here so the accepted
    # production entrypoint cannot expose Daily while silently omitting cohort
    # capture.
