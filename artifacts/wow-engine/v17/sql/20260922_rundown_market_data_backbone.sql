...
create table if not exists public.wow_market_price_observations (
    ...
    constraint wow_market_price_observations_available_price check (is_available = false or american_odds is not null)
);
...
create table if not exists public.wow_market_feed_sync_state (
    ...
    constraint wow_market_feed_sync_mode_check check (acquisition_mode in ('SNAPSHOT', 'DELTA', 'WEBSOCKET')),
    constraint wow_market_feed_sync_delay_check check (data_delay_seconds is null or data_delay_seconds >= 0),
    constraint wow_market_feed_sync_rows_check check (last_rows_written >= 0),
...
create table if not exists public.wow_market_provider_catalog (
    ...
    constraint wow_market_provider_catalog_type_check check (catalog_type in ('SPORT', 'MARKET', 'AFFILIATE')),
...
create policy wow_market_price_service_role
    on public.wow_market_price_observations for all to service_role
    using (true) with check (prediction_authority = false and can_execute = false);
...
create policy wow_market_sync_service_role
    on public.wow_market_feed_sync_state for all to service_role
    using (true) with check (can_execute = false);
...
create policy wow_market_catalog_service_role
    on public.wow_market_provider_catalog for all to service_role
    using (true) with check (can_execute = false);
