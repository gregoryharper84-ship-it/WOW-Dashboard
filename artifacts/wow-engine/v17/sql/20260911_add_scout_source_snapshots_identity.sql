...
create index if not exists idx_wow_scout_source_snapshots_sport_time on wow_scout.source_snapshots(sport_key, observed_at desc);
create index if not exists idx_wow_scout_entity_map_canonical on wow_scout.provider_entity_map(sport_key, entity_type, canonical_entity_id);
create index if not exists idx_wow_scout_candidate_source_link_status on wow_scout.candidate_source_links(candidate_id, link_status);
