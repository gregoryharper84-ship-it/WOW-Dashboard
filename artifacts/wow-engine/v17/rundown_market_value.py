...
It converts market quotes into market-only features and compares those features
with an already-completed calibrated probability package. It never scores a
sporting event, changes model probability, changes probability rank, or enables
execution.
"""
...
def american_implied_probability(odds: Any) -> float | None:
    ...
    This is deliberately *not* a sporting probability. For a single executable
    price it is the correct raw breakeven threshold. No-vig market probability
    is computed separately from paired prices at the same bookmaker.
    """
    price = _number(odds)
...
def _best_quote(quotes: Iterable[dict[str, Any]]) -> dict[str, Any] | None:
    usable = [q for q in quotes if _number(q.get("american_odds")) is not None]
    if not usable:
        return None
    # For American odds, numerically greater is always more bettor-favorable:
    # +120 beats +110 and -120 beats -135.
...
def _book_map(quotes: Iterable[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    for quote in quotes:
        ...
        if not book or _number(quote.get("american_odds")) is None:
            continue
        # If duplicate quotes exist for a book in one snapshot, retain the newest
        # provider timestamp. If timestamps are absent, retain the later input.
...
def _snapshot_features(snapshot: dict[str, Any] | None, *, now: datetime) -> dict[str, Any]:
    ...
    best_home = _best_quote(moneyline.get("home_quotes") or [])
    best_away = _best_quote(moneyline.get("away_quotes") or [])
    ...
    return {
        ...
        "median_no_vig_probability": {
            "home": home_stats["median"],
            "away": away_stats["median"],
        },
        "book_probability_range_pp": {
            "home": home_stats["range_pp"],
            "away": away_stats["range_pp"],
        },
        "book_probability_stddev_pp": {
            "home": home_stats["stddev_pp"],
            "away": away_stats["stddev_pp"],
        },
        "best_price": {
            "home": best_home,
            "away": best_away,
