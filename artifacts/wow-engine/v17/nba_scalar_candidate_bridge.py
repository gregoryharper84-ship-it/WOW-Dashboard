"""Evidence-only runtime bridge for NBA scalar fitted candidates.

The bridge gives exact NBA P/R/A scalar/composite CANDIDATE artifacts a narrow
research-scoring path so immutable exact-line evidence can accumulate.  It does
not provide calibrated probability, a lower bound, publication authority, ranking,
market approval, or execution authority.
"""
...
RESEARCH_STATS = frozenset(COMPONENTS_BY_STAT)


class NBAScalarCandidateBridgeError(RuntimeError):
...
def is_nba_scalar_candidate_request(req: Any) -> bool:
    sport = str(getattr(req, "sport", "") or "").strip().upper()
    try:
        stat = canonical_stat(str(getattr(req, "stat_type", "") or ""))
    except NBAScalarCandidateError:
        return False
    return sport == "NBA" and stat in RESEARCH_STATS
...
def _candidate_artifact(db: Any, *, stat_type: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise NBAScalarCandidateBridgeError(
            "NBA_SCALAR_CANDIDATE_ARTIFACT_REGISTRY_UNAVAILABLE",
            "NBA scalar candidate artifact registry lookup failed",
        ) from exc
    rows = [dict(row) for row in (result.data or [])]
    if not rows:
        raise NBAScalarCandidateBridgeError(
            "NBA_SCALAR_CANDIDATE_ARTIFACT_NOT_FOUND",
            f"No activated research candidate exists for NBA {stat}",
            failure_class="MODEL_UNAVAILABLE",
        )
    if len(rows) != 1:
        raise NBAScalarCandidateBridgeError(
            "NBA_SCALAR_CANDIDATE_ARTIFACT_AMBIGUOUS",
            f"Multiple research candidates resolved for NBA {stat}",
    ...
    if lifecycle not in ALLOWED_LIFECYCLES:
        blockers.append("NBA_SCALAR_CANDIDATE_LIFECYCLE_INVALID")
    if row.get("candidate_research_active") is not True:
        blockers.append("NBA_SCALAR_CANDIDATE_NOT_RESEARCH_ACTIVE")
    if any(row.get(key) is True for key in ("promoted", "active", "probability_publishable", "can_execute")):
        blockers.append("NBA_SCALAR_CANDIDATE_AUTHORITY_CONFLICT")
    if str(row.get("artifact_format") or "") != ARTIFACT_FORMAT:
        blockers.append("NBA_SCALAR_CANDIDATE_ARTIFACT_FORMAT_UNSUPPORTED")
    if str(row.get("specialist_version") or "").split("@", 1)[0] != CONTROLLING_SPECIALIST:
        blockers.append("NBA_SCALAR_CANDIDATE_SPECIALIST_MISMATCH")
    payload = row.get("artifact_payload")
    if not isinstance(payload, Mapping):
        blockers.append("NBA_SCALAR_CANDIDATE_PAYLOAD_INVALID")
    elif str(payload.get("exact_route_stat_type") or "").upper() != stat:
        blockers.append("NBA_SCALAR_CANDIDATE_ROUTE_IDENTITY_MISMATCH")
    if blockers:
        raise NBAScalarCandidateBridgeError(
            "NBA_SCALAR_CANDIDATE_ARTIFACT_INVALID",
            ",".join(blockers),
            failure_class="MODEL_OUTPUT_INVALID",
        )
    return row
...
def _http_failure(req: Any, exc: NBAScalarCandidateBridgeError) -> HTTPException:
    status = 409
...
def score_nba_scalar_candidate_research(market_api: Any, req: Any, *, model_identity: str) -> dict[str, Any]:
    if not is_nba_scalar_candidate_request(req):
        raise NBAScalarCandidateBridgeError("NBA_SCALAR_ROUTE_UNSUPPORTED", "Request is not a declared NBA scalar candidate route")
    stat = canonical_stat(str(getattr(req, "stat_type", "") or ""))
    try:
        ...
        raise _http_failure(req, NBAScalarCandidateBridgeError(
            "NBA_SCALAR_EVIDENCE_BRIDGE_FAILED",
            "NBA scalar evidence retrieval failed",
            failure_class="MODEL_SCORER_FAILED",
        )) from exc
    if evidence.get("ok") is not True or evidence.get("code") != "PROP_EVIDENCE_READY":
        raise _http_failure(req, NBAScalarCandidateBridgeError(
            str(evidence.get("code") or "NBA_SCALAR_EVIDENCE_INCOMPLETE"),
            "NBA scalar candidate inputs are incomplete",
    ...
    except NBAScalarCandidateBridgeError as exc:
        raise _http_failure(req, exc) from exc
    except NBAScalarCandidateError as exc:
        raise _http_failure(req, NBAScalarCandidateBridgeError(
            "NBA_SCALAR_CANDIDATE_SCORER_INVALID",
            str(exc),
            failure_class="MODEL_OUTPUT_INVALID",
        )) from exc
    except Exception as exc:
        raise _http_failure(req, NBAScalarCandidateBridgeError(
            "NBA_SCALAR_CANDIDATE_SCORER_EXCEPTION",
            "NBA scalar candidate scorer threw an unexpected exception",
            failure_class="MODEL_SCORER_FAILED",
        )) from exc
    ...
    return {
        ...
        "requested_scope": {"sport": "NBA", "stat_type": stat},
        "objective_lanes": {
            "MODEL": {"status": "PASS_RESEARCH_ONLY", "specialist_invoked": True, "can_execute": False},
            "CALIBRATION": {"status": "HOLD", "reason": "NBA_SCALAR_CANDIDATE_UNCALIBRATED", "can_execute": False},
...
__all__ = [
    "NBAScalarCandidateBridgeError", "RESEARCH_STATS", "candidate_preflight",
    "is_nba_scalar_candidate_request", "score_nba_scalar_candidate_research",
]
