...
def _login(page: Page) -> tuple[bool, str | None]:
    ...
    if _is_auth_page(page):
        # Do not echo auth-page errors: they can contain account-specific text.
...
def capture_board(output: str | Path) -> dict[str, Any]:
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            # A supplied storage state is accepted only if it changes the surface
            # from the known marketing landing page to the real account odds app.
            ...
            if auth_blocker is None:
                if _is_auth_page(page):
                    ...
                    # This is the critical anti-false-green guard. Static LAL/BOS,
                    # KC/BUF, etc. previews are never Scout inventory.
            ...
            if auth_blocker is None:
                ...
                if state_out:
                    # Caller must point this at an ephemeral runner path. The file
                    # is never uploaded by this collector.
