...
This is a fail-closed *data provenance* contract.  A source can be suitable for
research discovery, event identity, or market evidence without being authorized
for fitted-model training.  No source in this registry can publish a sporting
probability merely because it is available.

Commercial/license assertions are deliberately supplied at runtime rather than
assumed from a URL or credential.  This prevents a research/non-commercial feed
from silently entering production model artifacts.
"""
...
SOURCES: dict[str, SourceEntitlement] = {
    "THERUNDOWN": SourceEntitlement(
        ...
        "EVENT_MARKET_ONLY",
        credential_env=("RUNDOWN_API_KEY", "WOW_RUNDOWN_API_KEY", "THERUNDOWN_API_KEY"),
    ...
    "BALLDONTLIE": SourceEntitlement(
        ...
        ("NBA", "WNBA"),
        "TRAINING_WITH_CREDENTIAL",
        credential_env=("balldontlie", "BALLDONTLIE_API_KEY"),
    ...
    "MMA_APPROVED_STATS": SourceEntitlement(
        "MMA_APPROVED_STATS",
        ("MMA",),
        "NO_APPROVED_SOURCE",
        fitted_training_allowed_when_ready=False,
