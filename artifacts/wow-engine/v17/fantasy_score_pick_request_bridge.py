...
non-promoted research candidate. It also refines *genuine artifact absence* into
the exact V17 capability-manifest lifecycle blocker so known cross-sport build
gaps do not collapse into a generic artifact-missing result. Transport, registry,
RPC, scorer, and malformed-response failures are never rewritten.
"""
...
# Alias normalization is identity-only. It lets a board label reach the correct
# exact manifest/artifact key; it never grants model or publication authority.
...
def research_candidate_preflight(
    ...
    Only a validated Fantasy Score research artifact can receive the legacy
    compatibility-ready receipt. Every other route remains fail-closed. For a
    genuine artifact absence, the capability manifest may refine the blocker but
    can never set ``ok=true`` or make the probability publishable.
    """
