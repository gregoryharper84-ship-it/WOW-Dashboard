...
class _Query:
    ...
    def order(self, field, desc=False):
        self.rows.sort(key=lambda r: str(r.get(field) or ""), reverse=desc)
        return self
...
def test_materially_different_line_does_not_match():
    batch = PredictionReceiptLookupBatch.model_validate(
        {
            "rows": [
                {
...
def test_ambiguous_rows_are_not_guessed():
    second = _prediction(
...
def test_lookup_route_is_read_only_and_installs_once():
    ...
    install_prediction_receipt_lookup_route(app, db_client_fn=lambda: db, auth_dependency=dep)
    assert sum(getattr(r, "path", None) == "/v17/prediction-receipts/lookup" for r in app.router.routes) == 1
