...
batch (pre-hydration, canonical row preflight, /score-prop preflight, and fitted
provider resolution).  Those reads are governance checks, but their successful
results are immutable or slow-changing over the few seconds occupied by one
interactive request.

This module installs short-lived, in-process, positive-only caches. Failures,
missing rows, blocked capabilities, unsupported routes, and model exceptions are
never cached.  No probability, line, direction, calibration output, terminal
label, or execution authority is cached or changed.
"""
...
def clear_interactive_runtime_caches() -> None:
    """Test/maintenance helper. It never mutates any persisted V17 state."""
    with _CACHE_LOCK:
        _CACHE.clear()
        _KEY_LOCKS.clear()
...
def install_interactive_runtime_optimizations(market_api: Any) -> bool:
    """Install short positive-only caches around repeated registry reads.

    Returns True when the optimization is installed or already present. Any
    caller-visible failure still comes from the original governed function.
    """
