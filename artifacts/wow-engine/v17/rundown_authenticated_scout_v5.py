"""Final transport shim for the authenticated TheRundown Scout sweep.

v4 owns the validated login/readiness/full-slate normalization flow. This shim
changes only the request transport: never replay a captured Cookie header.
BrowserContext.request is allowed to attach the current authenticated cookie jar,
which is the exact no-header path the live transport probe repeatedly returned
HTTP 200 for. No secret values are logged or serialized. can_execute=false.
"""
...
def _current_session_json(context, _captured_headers: dict[str, str], path: str, params: dict[str, Any] | None = None) -> tuple[int, Any, str | None]:
    ...
    for attempt in range(base.MAX_RETRIES):
        try:
            # Do not pass Cookie/Authorization from the captured request. Playwright
            # attaches the context's current cookie jar, avoiding stale pre-login
            # cookies overriding the authenticated citizen_sessionid_v2 session.
...
def run() -> dict[str, Any]:
    ...
    payload["acquisition_transport"] = "PLAYWRIGHT_CURRENT_AUTHENTICATED_COOKIE_JAR"
