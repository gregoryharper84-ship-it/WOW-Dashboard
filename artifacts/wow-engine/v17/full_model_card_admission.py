...
TYPED_SCORER_FAILURES = {
    ...
    "CERTIFIED_ADAPTER_UNAVAILABLE",
    "COMPUTATION_VERIFICATION_FAILED",
    "COMPUTATION_VERIFICATION_CONFLICT",
}
...
def _lane(candidate: Mapping[str, Any]) -> str:
    ...
    aliases = {
        "PROP": PROP_LANE,
        "PROPS": PROP_LANE,
        "PLAYER_PROP": PROP_LANE,
        "PLAYER_PROPS": PROP_LANE,
        "TEAM_EVENT": TEAM_EVENT_LANE,
        "TEAM_EVENT_ML": TEAM_EVENT_LANE,
        "MONEYLINE": TEAM_EVENT_LANE,
        "ML": TEAM_EVENT_LANE,
        "OUTRIGHT_WINNER": TEAM_EVENT_LANE,
    }
...
def _typed_failure(payload: Mapping[str, Any] | None) -> str | None:
    if not payload:
        return None
    candidates = (
        payload.get("typed_failure"),
        payload.get("failure"),
        payload.get("failure_class"),
        payload.get("probability_claim_status"),
        payload.get("terminal_status"),
        payload.get("terminal_label"),
        payload.get("status"),
    )
...
def _route_supported(readiness: Mapping[str, Any]) -> bool | None:
    for key in ("route_supported", "model_capability_available", "capability_available"):
        ...
        if value is not None:
            return value
    capability = _norm(
        readiness.get("ncaaf_controlling_model")
        or readiness.get("model_capability")
        or readiness.get("capability_status")
    )
...
def _governed_package(scorer: Mapping[str, Any] | None) -> dict[str, Any]:
    """Copy only scorer-owned governed probability fields.

    This function intentionally has no access to candidate discovery probability
    fields, preventing external estimates from being relabeled as governed output.
    """
    if not scorer:
        return {}
    ...
    calibrated = _prob(scorer.get("calibrated_probability"))
    lower = _prob(scorer.get("calibrated_lower_bound"))
    upper = _prob(scorer.get("calibrated_upper_bound"))
...
def _valid_rank_package(package: Mapping[str, Any]) -> bool:
    ...
    if calibrated is None or lower is None:
        return False
    if lower > calibrated:
        return False
    ...
    if package.get("rank_eligible") is not True:
        return False
    terminal = _norm(package.get("terminal_label"))
    if terminal in REJECTING_TERMINALS:
        return False
    return True
...
def _sporting_probability_preserved(package: Mapping[str, Any]) -> bool:
    return any(
        _prob(package.get(key)) is not None
        for key in (
            "model_probability",
...
def admit_full_model_candidate(
    candidate: Mapping[str, Any],
    *,
    readiness: Mapping[str, Any] | None,
    scorer_result: Mapping[str, Any] | None,
) -> CardAdmissionDecision:
    """Fail-closed admission for one Full Model probability-card row."""
    ...
    eligible_for_scoring = (
        bool(candidate_id)
        and lane in SUPPORTED_LANES
        and bool(specialist)
        and trust_state not in TEST_ONLY_STATES
        and route_supported is not False
    )
    ...
    return CardAdmissionDecision(
        ...
        eligible_for_governed_scoring=eligible_for_scoring,
        admitted_to_probability_card=admitted,
        sporting_probability_preserved=sporting_preserved,
        governed_probability_package=package,
        terminal_label=terminal,
        typed_failure=typed_failure,
        blockers=tuple(dict.fromkeys(blockers)),
        market_status=market_status,
        external_research_present=external_present,
        can_execute=False,
    )


def admit_full_model_card(
    candidates: Iterable[Mapping[str, Any]],
    *,
    readiness_by_candidate: Mapping[str, Mapping[str, Any]],
    scorer_results_by_candidate: Mapping[str, Mapping[str, Any]],
) -> CardAdmissionBatch:
    """Reconcile every candidate exactly once into admitted or excluded."""
    ...
    for candidate in candidates:
        ...
        decision = admit_full_model_candidate(
            candidate,
