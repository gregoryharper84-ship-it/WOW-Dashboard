...
def odds_proxy_feed(
    *,
...
) -> Callable[[str, str], Iterable[Mapping[str, Any]]]:
    ...
    is used here only as one. A sport the proxy does not offer yields no rows;
    the caller records that as a source blocker rather than as an empty slate.
    """
    ...
    def fetch(sport: str, sport_key: str) -> list[Mapping[str, Any]]:
        result = proxy_get(
        ...
        if not getattr(result, "ok", False):
            raise RuntimeError(str(getattr(result, "code", None) or "EVENT_DISCOVERY_FAILED"))
...
def rundown_board_feed(
    *,
    ...
    """Discovery over TheRundown's current board for one slate date."""

    def fetch(sport: str, sport_key: str) -> list[Mapping[str, Any]]:
        result = live.get_sport_date_odds_snapshot(
            sport_key, slate_date, capability="events", opener=opener
        )
        if not result.ok:
            raise RuntimeError(str(result.code or "RUNDOWN_DISCOVERY_FAILED"))
...
def union_feed(
    *feeds: Callable[[str, str], Iterable[Mapping[str, Any]]],
) -> Callable[[str, str], Iterable[Mapping[str, Any]]]:
    ...
    def fetch(sport: str, sport_key: str) -> list[Mapping[str, Any]]:
        ...
        for feed in feeds:
            try:
                produced = list(feed(sport, sport_key) or ())
