...
def _compare_identity(leg: Mapping[str, Any], receipt: Mapping[str, Any]) -> list[str]:
    ...
    for dimension, aliases in _BASE_IDENTITY_ALIASES.items():
        left = _text(_first(leg, aliases)); right = _text(_first(receipt, aliases))
        if left is None or right is None: blockers.append(f"CARD_IDENTITY_INCOMPLETE:{dimension}")
        elif left != right: blockers.append(f"CARD_EXACT_IDENTITY_MISMATCH:{dimension}")
    for dimension, aliases in _OPTIONAL_IDENTITY_ALIASES.items():
        left_raw = _first(leg, aliases); right_raw = _first(receipt, aliases)
        if left_raw is None and right_raw is None: continue
        if left_raw is None or right_raw is None:
            blockers.append(f"CARD_EXACT_IDENTITY_MISMATCH:{dimension}"); continue
        if dimension == "exact_line":
            left = _decimal(left_raw); right = _decimal(right_raw)
            if left is None or right is None or left != right: blockers.append("CARD_EXACT_IDENTITY_MISMATCH:exact_line")
        elif _text(left_raw) != _text(right_raw): blockers.append(f"CARD_EXACT_IDENTITY_MISMATCH:{dimension}")
    return blockers
...
def gate_proposed_card(proposed_legs: Sequence[tuple[Mapping[str, Any], Mapping[str, Any] | None]], *, requested_leg_count: int | None = None, minimum_leg_count: int = 1) -> GovernedCardGateResult:
    ...
    if admitted_count < minimum_leg_count:
        portfolio_status = "HELD"; blockers.append("INSUFFICIENT_LEGS_AFTER_MANDATORY_SHRINK")
    elif shrunk: portfolio_status = "SHRUNK"
    else: portfolio_status = "PASS"
