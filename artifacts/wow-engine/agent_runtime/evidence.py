"""Evidence sealing and validation (packet section 11). Pure functions, no
database access. Ported near-verbatim from PR #33
...
from agent_runtime.idempotency import input_hash as _payload_hash_fn

REQUIRED_TOP_LEVEL = (
    "candidate_identity", "official_event", "exact_market_identity", "game_log",
    "box_score_log", "role_status", "role_timestamp", "source_attempts",
)
...
def validate_evidence_payload(payload: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    box_log = payload.get("box_score_log")
    ...
    if game_log is box_log:
        conflicts.append("GAME_LOG_BOX_SCORE_ALIAS_PROHIBITED")
    attempts = payload.get("source_attempts")
    if not isinstance(attempts, list) or not attempts:
        missing.append("source_attempts:nonempty")
    try:
        dt = datetime.fromisoformat(str(payload.get("role_timestamp")).replace("Z", "+00:00"))
        if dt.tzinfo is None:
            raise ValueError
    except Exception:
        missing.append("role_timestamp:aware_iso")
...
def seal_evidence(evidence_snapshot_id: str, payload: dict[str, Any]) -> SealedEvidence:
    ...
    return SealedEvidence(evidence_snapshot_id, payload, _payload_hash_fn(payload), tuple(missing), tuple(conflicts))
