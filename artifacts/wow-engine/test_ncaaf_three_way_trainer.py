...
def test_research_screen_never_changes_publication_state():
    candidate = train_calibrate_test(make_rows())
...
def test_three_way_rejects_leakage():
    rows = make_rows()
    bad = rows[10]
    rows[10] = TrainingRow(event_start_time=bad.event_start_time, feature_as_of=bad.event_start_time, home_won=bad.home_won, features=bad.features)
    with pytest.raises(NCAAFTrainingError) as exc:
