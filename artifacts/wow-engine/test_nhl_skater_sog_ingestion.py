...
def test_zero_sog_dressed_skater_is_a_legitimate_zero():
    result = sog.ingest_settled_game_skater_sog(_base_boxscore(), source_uri="u", retrieved_at="t")
...
def test_scratch_dnp_is_not_converted_into_a_zero_sog_observation():
    ...
    result = sog.ingest_settled_game_skater_sog(boxscore, scratches_raw=scratches, source_uri="u", retrieved_at="t")
...
def test_traded_player_resolves_to_updated_team_id_across_games():
    ...
    result_a = sog.ingest_settled_game_skater_sog(game_a, source_uri="u", retrieved_at="t")
    result_b = sog.ingest_settled_game_skater_sog(game_b, source_uri="u", retrieved_at="t")
...
def test_player_name_ambiguity_never_merges_distinct_player_ids():
    ...
    result = sog.ingest_settled_game_skater_sog(boxscore, source_uri="u", retrieved_at="t")
...
def test_duplicate_ingestion_is_idempotent_no_duplication():
    boxscore = _base_boxscore()
    result_1 = sog.ingest_settled_game_skater_sog(boxscore, source_uri="u", retrieved_at="t")
    result_2 = sog.ingest_settled_game_skater_sog(boxscore, source_uri="u", retrieved_at="t")
...
def test_deterministic_replay_produces_bit_identical_records():
    boxscore = _base_boxscore()
    result_1 = sog.ingest_settled_game_skater_sog(boxscore, source_uri="u", retrieved_at="t")
    result_2 = sog.ingest_settled_game_skater_sog(copy.deepcopy(boxscore), source_uri="u", retrieved_at="t")
...
def test_conflicting_content_under_same_canonical_key_is_rejected():
    ...
    result_2 = sog.ingest_settled_game_skater_sog(tampered_boxscore, source_uri="u", retrieved_at="t2")
    with pytest.raises(sog.NHLSogIngestionError) as exc:
...
def test_pregame_snapshot_excludes_facts_unavailable_before_puck_drop():
    ...
    early = sog.ingest_settled_game_skater_sog(early_game, source_uri="u", retrieved_at="t").records
    later = sog.ingest_settled_game_skater_sog(later_game, source_uri="u", retrieved_at="t").records
    future = sog.ingest_settled_game_skater_sog(future_game, source_uri="u", retrieved_at="t").records
...
def test_malformed_boxscore_missing_top_level_fields_fails_closed():
    ...
    with pytest.raises(sog.NHLSogIngestionError) as exc:
        sog.ingest_settled_game_skater_sog(incomplete, source_uri="u", retrieved_at="t")
