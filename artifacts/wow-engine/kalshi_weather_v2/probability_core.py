...
class WeatherProbabilityCore:
    ...
    Market prices are intentionally absent from this interface. Daily whole-
    degree extrema and hourly decimal Weather Index contracts deliberately use
    ...
    def build(
        ...
        *,
        ...
        if contract.lane not in self.SUPPORTED_LANES:
            return self._missing("PROBABILITY_LANE_UNSUPPORTED")
        ...
        if calibration.lane.strip().upper() != contract.lane:
            return self._missing("CALIBRATION_LANE_MISMATCH")
        ...
        source = (
            "KALSHI_WEATHER_V2_POINT_HORIZON_MODEL"
            if contract.lane == "HOURLY_TEMPERATURE"
            else "KALSHI_WEATHER_V2_STATION_HORIZON_MODEL"
        )
    ...
    def _event_probability(
        ...
        """Probability of the exact settlement predicate.

        Daily high/low contracts settle on whole-degree reported extrema, so
        those lanes use the existing ±0.5°F continuity correction. Hourly
        Kalshi Weather Index contracts use decimal structured strikes (for
        example a displayed "82° or above" can be represented as >81.99), so
        the exact strike is the probability boundary and no daily continuity
        correction is permitted.

        For same-day daily-high markets, condition on the official maximum
        already observed so far. Hourly point contracts never use this extrema
        truncation.
        """
        lower = contract.threshold_lower
        upper = contract.threshold_upper
        is_hourly = contract.lane == "HOURLY_TEMPERATURE"
        ...
        if observed_floor is None or is_hourly:
