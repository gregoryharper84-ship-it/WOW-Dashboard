...
from .contracts import RunStatus, canonical_hash
...
class MemoryStore:
    ...
    def get_run(self,run_id:str)->dict[str,Any]|None: return self.runs.get(run_id)
    ...
    def transition_run(self,run_id:str,nxt:RunStatus,stage:str|None=None)->dict[str,Any]:
        ...
        if nxt in {RunStatus.COMPLETED,RunStatus.COMPLETED_WITH_BLOCKERS,RunStatus.FAILED,RunStatus.CANCELED}:
...
class PostgresStore:
    ...
    def create_run(self, *, idempotency_key:str, request:dict[str,Any], governance_version:str)->dict[str,Any]:
        ...
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("select run_id,status,stage,rows_in,rows_completed,rows_held,rows_rejected,can_execute from wow.runs where idempotency_key=%s and request_hash=%s",(idempotency_key,request_hash))
            found=cur.fetchone()
            if found:
            ...
            cur.execute("""insert into wow.runs(run_id,idempotency_key,request_hash,request_payload,run_type,requested_as_of,user_timezone,status,stage,governance_version)
                           ...
                           returning run_id,status,stage,rows_in,rows_completed,rows_held,rows_rejected,can_execute""",
