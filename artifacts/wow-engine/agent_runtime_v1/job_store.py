...
class JobRepository:
    ...
    def claim(self, job_id:str)->bool:
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("""update wow.agent_jobs set status='RUNNING',started_at=coalesce(started_at,now()),heartbeat_at=now(),attempt=attempt+1
                           where job_id=%s and status in ('QUEUED','RETRY_PENDING')""",(job_id,))
    ...
    def complete(self, *, job_id:str, output:dict[str,Any])->bool:
        ...
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute("select run_id,candidate_id,worker_id,worker_version from wow.agent_jobs where job_id=%s for update",(job_id,))
            job=cur.fetchone()
            if not job: raise RuntimeError("JOB_NOT_FOUND")
            run_id,candidate_id,worker_id,worker_version=job
            ...
            if inserted:
                cur.execute("""update wow.agent_jobs set status=%s,output_hash=%s,ceiling=%s,blockers=%s::jsonb,completed_at=now(),heartbeat_at=now()
                               where job_id=%s and status='RUNNING'""",
                            (status.value,output["output_hash"],output.get("ceiling"),json.dumps(output.get("blockers") or []),job_id))
