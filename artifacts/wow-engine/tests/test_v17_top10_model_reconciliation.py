...
def test_ten_ranked_plus_one_unresolved_is_incomplete():
    ...
    outcomes.append({"row_key": "row-11", "terminal_status": "HELD", "code": "UNRESOLVED", "model_evaluated": False})
...
def test_typed_blockers_remain_in_denominator_without_filler():
    ...
    outcomes += [_blocker("row-10", "MODEL_INPUTS_INSUFFICIENT"), _blocker("row-11", "MODEL_SCORER_FAILED")]
    report = reconcile_top10_rows(rows, outcomes)
    assert report["rows_in_scope"] == 11
...
def test_missing_receipt_fails_exact_once_reconciliation():
    report = reconcile_top10_rows([_row(1), _row(2)], [_model("row-1")])
    assert report["balanced"] is False
...
def test_duplicate_receipt_fails_exact_once_reconciliation():
    report = reconcile_top10_rows([_row(1)], [_model("row-1"), _model("row-1")])
    assert report["balanced"] is False
...
def test_enforcement_blocks_completion_but_preserves_rows():
    rows = [_row(i) for i in range(1, 12)]
    outcomes = [_model(f"row-{i}") for i in range(1, 11)]
    outcomes.append({"row_key": "row-11", "terminal_status": "HELD", "code": "NOT_CALLED", "model_evaluated": False})
    original = {"ok": True, "run_controller_status": "DEGRADED", "reconciliation_pass": True, "rows": outcomes, "can_execute": False}
