                ...
                "markets": [
                    {
                        ...
                        "outcomes": [
                            ...
                            {"name": "Atlanta Falcons", "price": -105},
                        ],
                    }
                ],
            }
...
def _fail_live(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: _failed("SHARPAPI", "odds", "SHARPAPI_HTTP_429"),
    )
    monkeypatch.setattr(
        bridge.live,
        "rundown_market_evidence",
        lambda *args, **kwargs: _failed("RUNDOWN", "events", "RUNDOWN_HTTP_403", 403),
    )
...
def test_fresh_governed_snapshot_is_reused_when_live_providers_fail(monkeypatch, tmp_path):
    ...
    result = bridge.market_evidence_for_request(
        "/odds-api/v4/sports/americanfootball_nfl/events",
        None,
        {},
    ...
    row = result.data[0]
    assert row["id"] == "sharpapi-packers-falcons"
    marker = row["_wow_market_evidence"]
...
def test_cached_event_odds_resolve_via_locked_event_context(monkeypatch, tmp_path):
    ...
    result = bridge.market_evidence_for_request(
        "/odds-api/v4/sports/americanfootball_nfl/events/primary-provider-event-id/odds",
        None,
        context,
        primary_failure="ODDS_PROVIDER_NON_JSON:HTTP_429",
    )
...
def test_healthy_live_provider_does_not_mix_snapshot(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: sources.MarketEvidenceResult(
            True, "SHARPAPI", "odds", data=[live_event], status=200, code="MARKET_EVIDENCE_USED"
        ),
    )
    monkeypatch.setattr(
        bridge.live,
