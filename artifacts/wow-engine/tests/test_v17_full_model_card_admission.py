...
from v17.full_model_card_admission import admit_full_model_candidate, admit_full_model_card
...
def test_ncaaf_test_only_external_998_cannot_enter_full_model_card():
    ...
    readiness = _ready(
        ...
        blockers=["NCAAF_CERTIFIED_MODEL_ARTIFACT_NOT_FOUND", "NCAAF_CERTIFIED_CALIBRATOR_NOT_FOUND"],
    )
...
def test_unsupported_mls_external_projection_cannot_be_relabelled_governed():
    row = _candidate("inter-miami-atlanta", sport="SOCCER", external_probability=0.773, market_probability=0.71)
    decision = admit_full_model_candidate(row, readiness=_ready(route_supported=False, model_capability="MODEL_UNAVAILABLE"), scorer_result=None)
    assert decision.admitted_to_probability_card is False
    assert decision.governed_probability_package == {}
...
def test_external_probability_never_populates_governed_probability_fields():
    row = _candidate("external-only", sport="SOCCER", external_probability=0.91, external_lower_bound=0.87, no_vig_probability=0.88)
...
def test_valid_backend_package_is_required_for_probability_card_admission():
    decision = admit_full_model_candidate(_candidate("mlb-event", sport="MLB"), readiness=_ready(), scorer_result=_scored())
...
def test_rank_ineligible_sporting_probability_is_preserved_but_not_admitted():
    scorer = _scored(calibrated_probability=0.74, calibrated_lower_bound=0.69, calibrated_upper_bound=0.78, rank_eligible=False, model_qualified=False)
    decision = admit_full_model_candidate(_candidate("prop-calibration-held", lane="PROP", sport="MLB"), readiness=_ready(), scorer_result=scorer)
...
def test_missing_market_does_not_erase_valid_sporting_probability_or_rank_admission():
    decision = admit_full_model_candidate(_candidate("mlb-no-market", sport="MLB"), readiness=_ready(), scorer_result=_scored(market_status="NO_MARKET"))
    assert decision.sporting_probability_preserved is True
    assert decision.admitted_to_probability_card is True
...
def test_scorer_failure_is_preserved_and_not_rewritten_as_model_unavailable():
    scorer = {"terminal_label": "MODEL_SCORER_FAILED", "failure_class": "MODEL_SCORER_FAILED", "blockers": ["SCORER_TIMEOUT"]}
    decision = admit_full_model_candidate(_candidate("scorer-failed", lane="PROP", sport="MLB"), readiness=_ready(), scorer_result=scorer)
    assert decision.admitted_to_probability_card is False
...
def test_prop_forward_cohort_rank_hold_does_not_let_external_hit_rate_into_card():
    row = _candidate("gray-k-3.5", lane="PROP", sport="MLB", external_probability=0.80, recent_hit_rate=0.90)
    readiness = _ready(lane_status="PHASE_A_FORWARD_COHORT_BUILDING", probability_publishable=False, blockers=["PROP_CALIBRATION_HEALTH:PHASE_A_FORWARD_COHORT_BUILDING"])
    scorer = _scored(calibrated_probability=0.78, calibrated_lower_bound=None, calibrated_upper_bound=None, rank_eligible=False, model_qualified=False, probability_publishable=False)
...
def test_probability_publishable_false_blocks_admission_but_preserves_sporting_probability():
    scorer = _scored(probability_publishable=False, rank_eligible=True, calibrated_probability=0.71, calibrated_lower_bound=0.66)
    decision = admit_full_model_candidate(_candidate("publish-held", sport="MLB"), readiness=_ready(), scorer_result=scorer)
    assert decision.sporting_probability_preserved is True
    assert decision.admitted_to_probability_card is False
    assert "PROBABILITY_PUBLICATION_BLOCKED" in decision.blockers
...
def test_calibrated_only_payload_without_sporting_probability_is_rejected():
    ...
    decision = admit_full_model_candidate(_candidate("calibrated-only", sport="MLB"), readiness=_ready(), scorer_result=scorer)
    assert decision.admitted_to_probability_card is False
...
def test_readiness_publication_hold_blocks_even_if_row_payload_claims_rank_eligible():
    ...
    decision = admit_full_model_candidate(_candidate("readiness-held", sport="MLB"), readiness=readiness, scorer_result=_scored())
    assert decision.sporting_probability_preserved is True
    assert decision.admitted_to_probability_card is False
...
def test_batch_reconciles_every_candidate_exactly_once():
    rows = [_candidate("a", sport="MLB"), _candidate("b", sport="MLB"), _candidate("c", sport="SOCCER", external_probability=0.90)]
    batch = admit_full_model_card(
        rows,
        readiness_by_candidate={"a": _ready(), "b": _ready(), "c": _ready(route_supported=False, model_capability="MODEL_UNAVAILABLE")},
        scorer_results_by_candidate={"a": _scored(), "b": _scored(rank_eligible=False)},
    )
...
def test_batch_rejects_duplicate_candidate_ids():
    with pytest.raises(ValueError, match="DUPLICATE_CANDIDATE_ID"):
        admit_full_model_card(
            [_candidate("dup", sport="MLB"), _candidate("dup", sport="MLB")],
