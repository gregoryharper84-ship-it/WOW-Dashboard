...
def test_morning_green_still_blocks_merge_until_required_checks_and_scope_are_ready():
    ...
    assert "steps.gates.outputs.ready == 'true' && steps.scope.outputs.approved == 'true'" in workflow
    assert "gh pr merge" in workflow
    assert "can_execute: false" in workflow


def test_scope_rework_is_durable_but_does_not_fail_protected_main():
    repo_root = Path(__file__).resolve().parents[3]
    workflow = (
        repo_root / ".github" / "workflows" / "wow-v17-morning-green-continuation.yml"
    ).read_text(encoding="utf-8")

    assert 'echo "approved=false"' in workflow
    assert 'echo "guard_exit_code=$rc"' in workflow
    assert 'state="SCOPE_REWORK"' in workflow
    assert 'exit "$rc"' not in workflow
    assert "steps.scope.outputs.approved != 'true'" in workflow
