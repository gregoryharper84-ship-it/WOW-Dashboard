...
# api_prod_market resolves every governed prop artifact through this exact
# registry schema version. A 1IP-specific value here would create an artifact
# that can be stored but can never be found by the production route resolver.
...
# Columns accepted by the existing generic fitted-artifact registry. Review
# metadata and temporal lineage are carried inside validation_metrics JSONB so
# promotion requires no unreviewed schema expansion.
REGISTRY_COLUMNS = (
    "provider_identity",
    "model_family",
...
def build_empirical_shadow_candidate(
    *,
    ...
    lineage = {
        ...
        "validated_lines": list(SUPPORTED_LINES),
        "validation_metrics": metrics_with_support,
    }
...
def build_empirical_promotion_payload(
    ...
    *,
    ...
    """Create a review-approved payload without persisting or deploying it."""
    ...
    if shadow.get("feature_schema_version") != FEATURE_SCHEMA_VERSION:
        raise ValueError("MLB_1IP_PROMOTION_FEATURE_SCHEMA_MISMATCH")
    ...
    if tuple(float(v) for v in metrics.get("validated_lines") or ()) != SUPPORTED_LINES:
        raise ValueError("MLB_1IP_PROMOTION_VALIDATED_LINE_SUPPORT_MISMATCH")
...
def registry_row_from_promoted(promoted: dict[str, Any]) -> dict[str, Any]:
    """Serialize an approved packet to the existing registry table contract.

    Review evidence and validation lineage are embedded in validation_metrics
    JSONB; no extra database columns or migrations are required. This helper is
    pure and performs no database write.
    """
    if promoted.get("lifecycle_state") != "PROSPECTIVE_CERTIFIED":
        raise ValueError("MLB_1IP_REGISTRY_ROW_CERTIFICATION_REQUIRED")
    if promoted.get("promoted") is not True or promoted.get("active") is not True:
        raise ValueError("MLB_1IP_REGISTRY_ROW_PROMOTION_REQUIRED")
    if promoted.get("probability_publishable") is not False or promoted.get("can_execute") is not False:
        raise ValueError("MLB_1IP_PROMOTION_SAFETY_INVARIANT_VIOLATION")
    if promoted.get("feature_schema_version") != FEATURE_SCHEMA_VERSION:
        raise ValueError("MLB_1IP_PROMOTION_FEATURE_SCHEMA_MISMATCH")
    if tuple(float(v) for v in promoted.get("supported_lines") or ()) != SUPPORTED_LINES:
        raise ValueError("MLB_1IP_PROMOTION_VALIDATED_LINE_SUPPORT_MISMATCH")

    metrics = dict(promoted.get("validation_metrics") or {})
    metrics["validation_lineage"] = promoted.get("validation_lineage")
    metrics["review_evidence"] = promoted.get("review_evidence")
    metrics["validated_lines"] = list(SUPPORTED_LINES)

    source = dict(promoted)
    source["validation_metrics"] = metrics
    row = {key: source.get(key) for key in REGISTRY_COLUMNS}
    if set(row) != set(REGISTRY_COLUMNS):
        raise ValueError("MLB_1IP_REGISTRY_ROW_CONTRACT_INVALID")
