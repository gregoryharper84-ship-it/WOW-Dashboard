...
FEATURE_ORDER = (
    "week", "home_prior_games", "away_prior_games",
    "home_season_prior_games", "away_season_prior_games",
    "home_rest_days", "away_rest_days",
    "home_recent_off_epa_pp", "away_recent_off_epa_pp", "off_epa_edge",
    "home_recent_def_epa_pp", "away_recent_def_epa_pp", "def_epa_edge",
    "home_recent_success_rate", "away_recent_success_rate", "success_rate_edge",
    "home_recent_turnovers_pg", "away_recent_turnovers_pg", "turnover_edge",
    "home_recent_sacks_allowed_pg", "away_recent_sacks_allowed_pg", "sack_edge",
    "home_recent_st_epa_pg", "away_recent_st_epa_pg", "st_epa_edge",
    "home_prior_win_rate", "away_prior_win_rate", "win_rate_edge",
    "home_prior_point_diff_pg", "away_prior_point_diff_pg", "point_diff_edge",
)
...
def _prior_metrics(records: list[dict[str, Any]], *, target_date: date, season: int) -> dict[str, Any]:
    prior = sorted(
        (r for r in records if r["gameday"] < target_date),
        key=lambda r: (r["gameday"], r["game_id"]),
    )
    recent = prior[-ROLLING_GAMES:]
    season_prior = [r for r in prior if r["season"] == season]
    max_prior = prior[-1]["gameday"] if prior else None
    raw_rest = (target_date - max_prior).days if max_prior else None
    # A 200+ day offseason is not a meaningful linear rest advantage. Keep the
    # feature sensitive to short rest/byes while bounding offseason distortion.
    rest_days = min(max(raw_rest, 0), MAX_REST_DAYS) if raw_rest is not None else None
    ...
    return {
        ...
        "source_hashes": sorted({
            h for r in prior
            for h in (r["schedule_content_sha256"], r["pbp_content_sha256"])
            if h
        }),
    }
...
def build_prior_feature_rows(
    ...
    for game in ordered_games:
        ...
        features = {
            ...
            "home_prior_games": float(hm["prior_games"]), "away_prior_games": float(am["prior_games"]),
            "home_season_prior_games": float(hm["season_prior_games"]), "away_season_prior_games": float(am["season_prior_games"]),
            "home_rest_days": hm["rest_days"], "away_rest_days": am["rest_days"],
            "home_recent_off_epa_pp": hm["recent_off_epa_pp"], "away_recent_off_epa_pp": am["recent_off_epa_pp"],
            "off_epa_edge": _edge(hm["recent_off_epa_pp"], am["recent_off_epa_pp"]),
            "home_recent_def_epa_pp": hm["recent_def_epa_pp"], "away_recent_def_epa_pp": am["recent_def_epa_pp"],
            "def_epa_edge": _edge(am["recent_def_epa_pp"], hm["recent_def_epa_pp"]),
            "home_recent_success_rate": hm["recent_success_rate"], "away_recent_success_rate": am["recent_success_rate"],
            "success_rate_edge": _edge(hm["recent_success_rate"], am["recent_success_rate"]),
            "home_recent_turnovers_pg": hm["recent_turnovers_pg"], "away_recent_turnovers_pg": am["recent_turnovers_pg"],
            "turnover_edge": _edge(am["recent_turnovers_pg"], hm["recent_turnovers_pg"]),
            "home_recent_sacks_allowed_pg": hm["recent_sacks_allowed_pg"], "away_recent_sacks_allowed_pg": am["recent_sacks_allowed_pg"],
            "sack_edge": _edge(am["recent_sacks_allowed_pg"], hm["recent_sacks_allowed_pg"]),
            "home_recent_st_epa_pg": hm["recent_st_epa_pg"], "away_recent_st_epa_pg": am["recent_st_epa_pg"],
            "st_epa_edge": _edge(hm["recent_st_epa_pg"], am["recent_st_epa_pg"]),
            "home_prior_win_rate": hm["prior_win_rate"], "away_prior_win_rate": am["prior_win_rate"],
            "win_rate_edge": _edge(hm["prior_win_rate"], am["prior_win_rate"]),
            "home_prior_point_diff_pg": hm["prior_point_diff_pg"], "away_prior_point_diff_pg": am["prior_point_diff_pg"],
        ...
        row = {
            "game_id": str(game["game_id"]), "season": int(game["season"]), "week": int(game["week"]),
            "gameday": target_date.isoformat(), "home_team": home, "away_team": away,
            "feature_schema_version": FEATURE_SCHEMA_VERSION,
            "feature_cutoff_date": target_date.isoformat(), "max_prior_gameday": max_prior,
            "feature_order": list(FEATURE_ORDER), "features": features, "feature_vector": vector,
            "target_outcome": target, "home_prior_games": hm["prior_games"], "away_prior_games": am["prior_games"],
            "training_eligible": not exclusions, "exclusion_reasons": sorted(set(exclusions)),
