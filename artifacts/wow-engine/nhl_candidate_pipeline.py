"""NHL regular-season historical reconstruction and governed D1 candidate fitting.
...
not immutable archived pregame observations, so outputs remain CANDIDATE-only
until independent source review and prospective evidence are complete.
"""
...
TEAM_ABBREVIATIONS = tuple(sorted(set(BASE_TEAMS + ("ARI", "UTA"))))
...
def teams_for_start_year(start_year: int) -> tuple[str, ...]:
    """Return the exact active-team universe for modern training seasons.

    Arizona played through 2023-24; Utah begins with 2024-25. This prevents the
    same franchise history from being requested under both abbreviations in the
    same season and makes season membership auditable.
    """
    year = int(start_year)
    teams = set(BASE_TEAMS)
    if year <= 2023:
        teams.add("ARI")
    else:
        teams.add("UTA")
...
def normalize_game(raw: Mapping[str, Any], *, source_uri: str, retrieved_at: str) -> NHLGame | None:
    ...
    return NHLGame(
        ...
        event_start_time=start_utc.isoformat(),
...
def fetch_team_season_games(
    ...
    *,
    ...
    if abbr not in teams_for_start_year(start_year):
        raise NHLCandidateError("NHL_TEAM_NOT_ACTIVE_IN_SEASON", f"{abbr}:{start_year}")
    ...
    expected_sid = season_id(start_year)
    mismatched = [game.game_id for game in normalized if game.season_id != expected_sid]
    if mismatched:
        raise NHLCandidateError("NHL_SOURCE_SEASON_IDENTITY_DRIFT", mismatched[0])
    return normalized
...
def fetch_historical_games(
    ...
    *,
    ...
    explicit_teams = tuple(teams) if teams is not None else None
    for year in sorted({int(v) for v in start_years}):
        season_teams = explicit_teams or teams_for_start_year(year)
...
def candidate_record(candidate: BinaryCandidate, *, training_code_sha: str) -> dict[str, Any]:
    sha = str(training_code_sha or "").strip()
    if len(sha) < 7:
    ...
    version = f"NHL_RS_LOGISTIC_V1-{candidate.dataset_hash[:16]}-{sha[:12]}"
...
def build_candidate(start_years: Iterable[int], *, training_code_sha: str, session: Any = requests) -> dict[str, Any]:
    games = fetch_historical_games(start_years, session=session)
    candidate, features = fit_nhl_candidate(games)
    return {
        "status": "CANDIDATE_BUILT",
        "source_id": SOURCE_ID,
        "games": [game_record(game) for game in games],
        "feature_rows": features,
        "candidate": candidate_record(candidate, training_code_sha=training_code_sha),
