...
def validate_github_actions_claims(claims: dict[str, Any]) -> dict[str, Any]:
    ...
    if str(claims.get("event_name") or "") not in ALLOWED_EVENTS:
        raise GitHubOIDCValidationError("GITHUB_OIDC_EVENT_NOT_ALLOWED")
    return dict(claims)


def _internal_service_auth(token: str) -> dict[str, Any] | None:
    # Same read-only bearer is forwarded unchanged to the primary odds proxy.
    # It is separate from Custom GPT Action credentials and has no execution lane.
...
def verify_github_actions_oidc(
    ...
    *,
    ...
    try:
        ...
        raise GitHubOIDCValidationError(
            "GITHUB_OIDC_SIGNATURE_OR_STANDARD_CLAIMS_INVALID"
        ) from exc
