...
def test_sports_inventory_is_read_only_and_server_keyed(monkeypatch, configured):
    ...
    def fake_get(url, params):
        ...
        return FakeResponse(200, [{"key": "basketball_nba", "active": True}])

    monkeypatch.setattr(api, "_http_get", fake_get)
    client = TestClient(api.app)
    response = client.get("/odds-api/v4/sports", params={"all": "true", "apiKey": "bad"}, headers=configured)
    assert response.status_code == 200
    assert captured["url"] == "https://api.the-odds-api.com/v4/sports"
    assert captured["params"] == {"all": "true", "apiKey": "vendor-secret-test"}
    assert response.json()[0]["key"] == "basketball_nba"
...
def test_events_appends_vendor_key_server_side_and_discards_unknown_query(monkeypatch, configured):
    ...
    response = client.get(
        ...
        params={"dateFormat": "iso", "includeRotationNumbers": "true", "apiKey": "caller-should-not-control-this", "url": "https://evil.example"},
        headers=configured,
    )
    assert response.status_code == 200
...
def test_event_markets_requires_regions_or_bookmakers(configured):
    client = TestClient(api.app)
    response = client.get("/odds-api/v4/sports/baseball_mlb/events/abc123/markets", headers=configured)
...
def test_event_markets_forwards_only_allowlisted_params(monkeypatch, configured):
    ...
    response = client.get(
        ...
        params={"regions": "us", "dateFormat": "iso", "unexpected": "drop-me"}, headers=configured,
    )
    assert response.status_code == 200
    assert captured["url"].endswith("/sports/baseball_mlb/events/abc123/markets")
    assert captured["params"] == {"regions": "us", "dateFormat": "iso", "apiKey": "vendor-secret-test"}
...
def test_event_odds_forwards_market_options(monkeypatch, configured):
    ...
    def fake_get(url, params):
        captured["url"] = url
        captured["params"] = params
        return FakeResponse(200, {"id": "abc123", "bookmakers": []}, {"x-requests-remaining": "80", "x-requests-used": "20", "x-requests-last": "1"})

    monkeypatch.setattr(api, "_http_get", fake_get)
    client = TestClient(api.app)
    response = client.get(
        "/odds-api/v4/sports/baseball_mlb/events/abc123/odds",
        params={"markets": "h2h,pitcher_strikeouts", "regions": "us", "oddsFormat": "american", "includeLinks": "false"}, headers=configured,
    )
    assert response.status_code == 200
...
def test_upstream_error_is_sanitized_and_vendor_key_redacted(monkeypatch, configured):
    def fake_get(url, params):
        return FakeResponse(401, {"message": "bad key vendor-secret-test for request https://example.test?apiKey=vendor-secret-test"})

    monkeypatch.setattr(api, "_http_get", fake_get)
    client = TestClient(api.app)
    response = client.get("/odds-api/v4/sports/baseball_mlb/events", headers=configured)
...
def test_missing_vendor_key_fails_closed(monkeypatch):
    monkeypatch.setenv("WOW_ODDS_PROXY_ACTION_KEY", "caller-secret-test")
    client = TestClient(api.app)
    response = client.get("/odds-api/v4/sports/baseball_mlb/events", headers={"Authorization": "Bearer caller-secret-test"})
...
def test_no_write_method_exists(configured):
    client = TestClient(api.app)
    assert client.post("/odds-api/v4/sports", headers=configured).status_code == 405
    assert client.post("/odds-api/v4/sports/baseball_mlb/events", headers=configured).status_code == 405
