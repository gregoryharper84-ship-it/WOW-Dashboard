...
- Callers authenticate separately with ``WOW_ODDS_PROXY_ACTION_KEY`` via a
  Bearer header.
- Only three explicitly defined GET capabilities are proxied: events, event
  markets, and event odds. There is no generic URL/path proxy and no write
  method.
...
app = FastAPI(title="WOW Odds API Credential Proxy", version="1.0.0")
...
def _require_proxy_action_key(authorization: Optional[str] = Header(default=None)) -> None:
    ...
    if not configured:
        raise HTTPException(
            status_code=503,
            detail={"code": "ODDS_PROXY_AUTH_UNCONFIGURED", "can_execute": False},
        )
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(
            status_code=401,
            detail={"code": "ODDS_PROXY_AUTH_REQUIRED", "can_execute": False},
        )
...
def _vendor_key() -> str:
    ...
    if not key:
        raise HTTPException(
            status_code=503,
            detail={"code": "ODDS_API_KEY_UNCONFIGURED", "can_execute": False},
        )
    return key
...
def _safe_upstream_message(response: httpx.Response) -> Optional[str]:
    """Return a short vendor message without reflecting URL/query credentials."""
    try:
...
def _proxy_get(upstream_path: str, params: dict[str, str]) -> JSONResponse:
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail={"code": "ODDS_API_UPSTREAM_UNREACHABLE", "can_execute": False},
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail={"code": "ODDS_API_UPSTREAM_NON_JSON", "can_execute": False},
        )
...
def _require_regions_or_bookmakers(regions: Optional[str], bookmakers: Optional[str]) -> None:
    if not regions and not bookmakers:
        raise HTTPException(
            status_code=422,
            detail={
                "code": "REGIONS_OR_BOOKMAKERS_REQUIRED",
                "message": "Provide regions or bookmakers for this Odds API request.",
                "can_execute": False,
            },
        )
...
def health():
    return {
        "status": "ok",
        "service": "WOW_ODDS_API_CREDENTIAL_PROXY",
        "compute_provider": "RENDER",
        "vendor": "THE_ODDS_API_V4",
        "read_only": True,
        "vendor_key_configured": bool(os.environ.get("ODDS_API_KEY")),
        "caller_auth_configured": bool(os.environ.get("WOW_ODDS_PROXY_ACTION_KEY")),
        "can_execute": False,
    }
...
@app.get(
    ...
    dependencies=[Depends(_require_proxy_action_key)],
)
def get_events(
    sport: str = SportPath,
    date_format: Literal["iso", "unix"] = Query("iso", alias="dateFormat"),
    event_ids: Optional[str] = Query(
        None,
        alias="eventIds",
        pattern=r"^[A-Za-z0-9_-]+(?:,[A-Za-z0-9_-]+)*$",
        max_length=4096,
    ...
    params = _clean_params(
        ...
        includeRotationNumbers=include_rotation_numbers,
    )
...
def get_event_odds(
    ...
    include_sids: Optional[bool] = Query(None, alias="includeSids"),
    ...
    params = _clean_params(
        ...
        includeRotationNumbers=include_rotation_numbers,
        includeMultipliers=include_multipliers,
    )
