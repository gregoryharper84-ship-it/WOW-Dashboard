...
| WOW-PATCH-2026-07-06-CROSS-MARKET-REJECT-PROOF-AND-DEGRADED-RUN-GATE | 2026-07-06 | SHIPPED | Section 33 — legacy `/wow-daily-scan` (`jobs/wow_daily_scan.py`) gains full reject-proof output contract (board/consensus line, no-vig probability, edge math, drift grade), `REJECT_NO_EDGE` explicit math blocker, 9 mandatory `market_cause` tags, scoped `SOURCE_CONFLICT` cross-bookmaker check, reused pitcher-deployment signal, WNBA L5/L10 confirmed already generic, PP whole-line cash-threshold conversion, scoped same-player mutex grouping, and `DEGRADED_ENGINE_RUN` gate (upstream fetch failures force playable buckets to `watch` with `degraded_run_hold: true` instead of a bare 500). New `jobs/market_math.py` helper module. 17 new non-destructive `scan_results` columns. Full Layer-0 reconciliation, full pitcher-deployment module, and full correlation guard deliberately deferred (documented scope decision). Live-verified via `/wow-daily-scan`. |
| WOW-PATCH-2026-07-07-JS-STYLE-CONVERSION-LAYER | 2026-07-07 | SHIPPED | JS-Style Conversion Layer — `gate_engine/js_style_conversion.py`; `run(row, enrichment)` per-row gate + `run_slip(rows)` slip-level gate; wired into `pipeline.py` after `directional_exposure.run()` / before `slip_structure.run_single()` and after `slip_structure.run_slip()`; 8 sub-tag labels (JS_VALID / JS_WATCH_FAKE_DISCOUNT / JS_CLOSE_NO_UPGRADE / JS_REJECT_BAD_STRUCTURE / JS_REJECT_MARKET_CONFLICT / JS_REJECT_LOW_K_LESS_TRAP / JS_REJECT_SAME_GAME_PRA_CLUSTER / JS_REJECT_THIN_CUSHION); 4 hard gates (A: WNBA same-game PRA MORE cluster without full env support, B: pitcher outs-MORE + K-LESS conflict without contact-outs proof, C: MLB K LESS ≤4.0 without restriction proof, D: cushion below market-specific hard floor); advisory thin-cushion range [0.5, 1.0] → JS_CLOSE_NO_UPGRADE (not hard-rejected, slip_structure_allowed=True); slip-level duplicate-exposure counter + same_game_cluster_id; 17/17 tests pass. |
| WOW-PATCH-2026-07-07-KALSHI-FINAL-LOCK-EDGE-DISCOVERY | 2026-07-07 | SHIPPED | Section 34 — Kalshi sports lane enforcement: (1) Gate A: server-side direct Kalshi API orderbook fetch only — caller-supplied/web prices cap at LLP_WATCH (`KALSHI_ORDERBOOK_SOURCE_NOT_DIRECT_API`); (2) Gate B: market trading_active gate — closed/settled markets cap at LLP_SCOUT (`MARKET_NOT_TRADING`); (3) Gate C: final-lock recheck freshness window (30 min) — absent/stale cap at LLP_WATCH (`FINAL_LOCK_RECHECK_REQUIRED`); (4) market-type-aware edge floors: KXMLBGAME/KXWNBAGAME = 1.5% (EDGE_FLOOR_MAIN), derivative = 2.5% (EDGE_FLOOR_DERIVATIVE); (5) new `kalshi_candidate_ledger` table logs all WATCH/PLAYABLE/SCOUT candidates for CLV tracking; (6) new `orderbook_fetcher.py` + `kalshi_watch_ledger.py` modules; 60/60 tests pass (6 new regression + 54 existing). |
