...
| WOW-PATCH-010 | 2026-07-01 | SHIPPED | Market Data Contract Registry — `POST /wow/data-contract/check` + `GET /wow/data-contract/registry`; 7 WNBA markets (PTS/REB/AST/PRA/P+A/P+R/R+A); logical field aliases (player_id_or_name, starter_or_role, ingestion_ts); status labels (COMPLETE/INCOMPLETE/STALE/PARTIAL/DATA_BUILD_PRIORITY); L5/L10 row-count window check; approval_ceiling enforced per status; Gate 3 untouched |
