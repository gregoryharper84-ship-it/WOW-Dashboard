...
class TestGitDiffAllowlist(unittest.TestCase):
    ...
    _ISOLATED_STAGE_A_TRAILER = "WOW-STAGE-A-ISOLATED: true"

    # Deterministic context for the rule tests below.  These tests validate
    # the Stage-A rules themselves; they must not depend on whichever commit
    # happens to be checked out when normal CI runs.
    _VALID_ISOLATED_STAGE_A_PATHS: tuple[str, ...] = (
        "gate_engine/prob_ledger_enforcer.py",
        "gate_engine/outlier_recompute.py",
        "gate_engine/tests/test_prob_ledger_enforcer.py",
        "gate_engine/tests/test_outlier_recompute.py",
    ...
    @classmethod
    def _is_explicitly_isolated_stage_a_commit(
        cls, commit_message: str, changed: list[str] | tuple[str, ...]
    ) -> bool:
        """Classify real git context only with an explicit isolated marker."""
        marker = cls._ISOLATED_STAGE_A_TRAILER.casefold()
        has_marker = any(
            line.strip().casefold() == marker
            for line in commit_message.splitlines()
        )
        return has_marker and any(
            path.replace("\\", "/") in cls._STAGE_A_TRIGGER_FILES
            for path in changed
        )
    ...
    def _normalized_paths(
        ...
        return [path.strip().replace("\\", "/") for path in changed if path.strip()]

    def _assert_allowlisted_stage_a_diff(
        self, changed: list[str] | tuple[str, ...]
    ):
        changed = self._normalized_paths(changed)
        self.assertTrue(
            any(path in self._STAGE_A_TRIGGER_FILES for path in changed),
            "test context must include a Stage-A production trigger",
        )
        violations = [path for path in changed if not self._is_allowed(path)]
        self.assertListEqual(
            violations,
            [],
            f"Stage A context touched forbidden file(s): {violations}\n"
            f"All changed files: {changed}",
        )

    def _assert_no_hard_fail_patterns(
        self, changed: list[str] | tuple[str, ...]
    ):
        changed = self._normalized_paths(changed)
        for pattern in self._HARD_FAIL_PATTERNS:
            hits = [path for path in changed if pattern in path]
            self.assertListEqual(
                hits,
                [],
                f"HARD FAIL: Stage A context contains prohibited "
                f"pattern {pattern!r}: {hits}",
            )

    def _assert_no_prohibited_filenames(
        self, changed: list[str] | tuple[str, ...]
    ):
        changed = self._normalized_paths(changed)
        for filename in self._PROHIBITED_FILENAMES:
            hits = [
                path for path in changed
                if pathlib.PurePosixPath(path).name == filename
            ]
            self.assertListEqual(
                hits,
                [],
                f"HARD FAIL: Stage A context contains prohibited "
                f"file {filename!r}: {hits}",
            )

    def test_most_recent_commit_only_touches_allowed_files(self):
        """
        An isolated Stage-A context must only touch allowlisted files.

        Real commit classification is tested separately.  Keeping this rule
        test on a deterministic context prevents a full-source migration
        squash from being mistaken for the isolated Stage-A patch.
        """
        self._assert_allowlisted_stage_a_diff(self._VALID_ISOLATED_STAGE_A_PATHS)
    ...
    def test_hard_fail_patterns_not_in_diff(self):
        """
        An isolated Stage-A context must contain no hard-fail patterns.
        """
        self._assert_no_hard_fail_patterns(self._VALID_ISOLATED_STAGE_A_PATHS)

    def test_prohibited_filenames_not_in_diff(self):
        """
        An isolated Stage-A context must contain no prohibited basenames.
        """
        self._assert_no_prohibited_filenames(self._VALID_ISOLATED_STAGE_A_PATHS)

    def test_git_context_requires_explicit_isolated_marker(self):
        """
        A migration/source-squash context is not an isolated Stage-A patch
        merely because it contains Stage-A production files.
        """
        squash_paths = self._VALID_ISOLATED_STAGE_A_PATHS + (
            "gate_engine/pipeline.py",
            "artifacts/flask-scoring-api/app.py",
        )
        migration_message = "migration: import sanitized WOW source state"
        self.assertFalse(
            self._is_explicitly_isolated_stage_a_commit(
                migration_message, squash_paths
            )
        )
        isolated_message = (
            "WOW-PATCH-2026-08-10-STAGE-A-PROBABILITY-LEDGER-OUTLIER-RECOMPUTE\n"
            f"{self._ISOLATED_STAGE_A_TRAILER}"
        )
        self.assertTrue(
            self._is_explicitly_isolated_stage_a_commit(
                isolated_message, self._VALID_ISOLATED_STAGE_A_PATHS
            )
        )
