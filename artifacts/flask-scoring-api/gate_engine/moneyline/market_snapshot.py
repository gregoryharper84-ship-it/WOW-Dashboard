...
def _is_stale(retrieved_at: str | None, now: datetime, max_age_hours: float) -> bool:
    """Missing, malformed, future-dated, and old quote provenance is unusable."""
    if not retrieved_at:
        return True
    try:
    ...
    return age_seconds < 0 or age_seconds > max_age_hours * 3600.0


def revalidate_snapshot_freshness(
    snap: MoneylineMarketSnapshot,
    *,
    now: datetime | None = None,
    max_quote_age_hours: float = DEFAULT_MAX_QUOTE_AGE_HOURS,
) -> list[BookQuote]:
    """Remove stale supplied quotes before they can be handed to the scorer.

    Snapshots can be serialized and reused after the acquisition request.  This
    second check is intentionally at the consumer boundary so neither a direct
    object nor a deserialized snapshot can bypass quote freshness.
    """
    now = now or datetime.now(timezone.utc)
    fresh: list[BookQuote] = []
    for quote in snap.books:
        if _is_stale(quote.retrieved_at, now, max_quote_age_hours):
            snap.dropped.append({
                "bookmaker": quote.bookmaker,
                "reason": "STALE_SNAPSHOT_QUOTE",
        ...
        fresh.append(quote)
    snap.books = fresh
    snap.counters["books_fresh"] = len({quote.bookmaker for quote in fresh})
    return fresh
...
def attach_snapshot_to_enrichment(
    ...
    revalidate_snapshot_freshness(snap)
    if not snap.books:
        return _breach(
            "STALE_OR_EMPTY_SNAPSHOT_SUPPLIED",
            books_fetched=snap.counters.get("books_fetched", 0),
        )
    ...
    if snap.books:
        ...
        home_no_vig = consensus_no_vig(snap, snap.home_team)
        away_no_vig = consensus_no_vig(snap, snap.away_team)
        if (
            home_no_vig is None
            or away_no_vig is None
            or abs((home_no_vig + away_no_vig) - 1.0) > 0.01
        ):
            return _breach(
                "NO_VIG_PAIR_NOT_RECONCILED",
