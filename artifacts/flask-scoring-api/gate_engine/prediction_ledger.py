...
def write_prediction(
    ...
    cal_prob  = (
        _safe_float(specialist.get("calibrated_probability"))
        or _safe_float(wnba_g.get("cal_selected"))
        or _safe_float(tennis_g.get("cal_selected"))
        or _safe_float(row.get("calibrated_probability"))
    )
    lower_bound = (
        _safe_float(specialist.get("calibrated_probability_lower_bound"))
        or _safe_float(specialist.get("lower_bound"))
        or _safe_float(wnba_g.get("cal_lower_bound"))
        or _safe_float(tennis_g.get("cal_lower_bound"))
        or _safe_float(row.get("calibrated_probability_lower_bound"))
    )
    upper_bound = (
        _safe_float(specialist.get("calibrated_probability_upper_bound"))
        or _safe_float(specialist.get("upper_bound"))
        or _safe_float(wnba_g.get("cal_upper_bound"))
        or _safe_float(row.get("calibrated_probability_upper_bound"))
    )
    raw_prob = (
        _safe_float(specialist.get("independent_probability"))
        or _safe_float(specialist.get("raw_probability"))
        or _safe_float(wnba_g.get("raw_selected"))
    ...
    # Persist the exact signed envelope, never a reconstructed subset.  The
    # DB ranker verifies it before use, so a row stripped of this object fails
    # closed rather than falling back to ordinary-prop ranking.
    if specialist:
        meta["specialist_probability"] = specialist
    for key in ("controlling_skill", "market_family", "objective"):
        if row.get(key) is not None:
            meta[key] = row.get(key)
    from gate_engine.moneyline.routing_policy import is_llp_moneyline_candidate
    if is_llp_moneyline_candidate(row):
        # A canonical h2h/moneyline input cannot silently become a generic
        # prop if a producer omitted its specialist metadata or handoff.
        meta["llp_moneyline_routing_required"] = True
