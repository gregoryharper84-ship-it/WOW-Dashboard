...
def classify_legs(legs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    # ── Rule 5: shared_thesis_keys via CorrelationGuard ──────────────────────
    # The CorrelationGuard from wow_fantasy_score emits canonical per-slip keys
    # for Fantasy Score / component-stat pairs.  These keys are added to the
    # cross-slip exposure ledger so the portfolio governor can detect when two
    # separate slips both carry the same player's FS exposure.
    shared_thesis_keys: list[str] = []
    try:
        from .wow_fantasy_score.correlation import (
            CorrelationGuard as _CG, PropLeg as _PL,
        )
        _guard = _CG()
        for i in range(n):
            for j in range(i + 1, n):
                ...
                pl_i = _PL(
                    leg_id    = str(i),
                    sport     = _normalize(li.get("sport") or ""),
                    player    = li.get("player") or "",
                    event_key = _normalize(li.get("event_key") or li.get("team") or ""),
                    stat_type = li.get("stat") or li.get("prop_type") or "",
                    direction = (li.get("side") or "MORE").upper(),
                    line      = float(li.get("line") or 0.0),
                )
                pl_j = _PL(
                    leg_id    = str(j),
                    sport     = _normalize(lj.get("sport") or ""),
                    player    = lj.get("player") or "",
                    event_key = _normalize(lj.get("event_key") or lj.get("team") or ""),
                    stat_type = lj.get("stat") or lj.get("prop_type") or "",
                    direction = (lj.get("side") or "MORE").upper(),
                    line      = float(lj.get("line") or 0.0),
                )
                decision = _guard.evaluate_pair(pl_i, pl_j)
                shared_thesis_keys.extend(decision.shared_thesis_keys)
    except Exception:
        pass  # fail-open: cross-slip keys are additive; absence doesn't block scoring
...
def _clean_result(
    ...
    block_power: bool,
    note_flex: bool,
    shared_thesis_keys: list[str] | None = None,
) -> dict[str, Any]:
    return {
        ...
        "can_approve_bets":   False,
        # shared_thesis_keys: canonical cross-slip exposure keys for portfolio governor.
        # Format: "{player}:{event_key}:{sport_family}_FANTASY_SCORE"
        # Generated by CorrelationGuard for Fantasy Score / component-stat pairs.
        "shared_thesis_keys": list(dict.fromkeys(shared_thesis_keys or [])),  # dedupe, preserve order
    }
