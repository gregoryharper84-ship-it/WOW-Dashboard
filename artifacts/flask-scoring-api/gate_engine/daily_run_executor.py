...
def execute_run(
    *,
    ...
    try:
        ...
        # Scope values are read ONLY from the persisted manifest identity —
        # a retry or restarted executor can never inject a different scope.
        orchestrator(
            run_id=run_id,
            ...
            persist=True,
            execution_owner=execution_owner,
            scope=str(run.get("request_scope") or "FULL_BOARD"),
            scope_requested_at=_as_iso(run.get("scope_requested_at")),
            run_timezone=str(run.get("run_timezone") or "UTC"),
        )
