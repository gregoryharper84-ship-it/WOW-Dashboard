...
_OPERATOR_DAILY_TERMINAL_STATUSES = frozenset({
    "COMPLETE", "DEGRADED", "NO_PLAY", "ENGINE_ERROR", "DATA_ERROR",
})


def operator_daily_status(
    raw_status: str | None,
    *,
    ...
    """Map internal Daily outcomes to the bounded operator vocabulary.

    ACCEPTED and IN_PROGRESS remain visible while the detached executor owns
    the run.  Terminal detail stays in the existing failure/reconciliation
    fields, while callers receive one actionable terminal status.
    """
    status = str(raw_status or "").upper()
    if status in {"ACCEPTED", "IN_PROGRESS"}:
        return status
    if (
        status in {"RECONCILIATION_WARNING", "RECONCILIATION_FAILED"}
        or (isinstance(reconciliation, dict) and reconciliation.get("reconciled") is False)
    ):
        return "DATA_ERROR"
    failure_text = " ".join(
        str(value or "").upper()
        for value in (failure_reason, failure_module)
    ).strip()
    if (
        status in {"FAILED", "ENGINE_ERROR"}
        or any(token in failure_text for token in (
            "RUNNER_", "DEADLINE", "HEARTBEAT", "OWNERSHIP", "ENGINE",
        ))
    ):
        return "ENGINE_ERROR"
    if status == "PARTIAL":
        return "DEGRADED"
    if status in {"DEGRADED", "DATA_ERROR"}:
        return status
    if status in {"COMPLETE", "NO_PLAY"}:
        playable = (counts or {}).get("playable_count")
        if status == "COMPLETE" and playable == 0 and not failure_text:
            return "NO_PLAY"
        return status
    if "PERSIST" in failure_text or "DATA" in failure_text:
        return "DATA_ERROR"
    return "ENGINE_ERROR"
...
def wnba_ml_specialist_ready(enrichment_entry: dict[str, Any]) -> bool:
    """
    True only for a provenance-bearing standings pair that can support the
    WNBA_ML_V1 independent-probability specialist. Partial, un-timestamped,
    or source-less values never activate the specialist.
    """
    if not isinstance(enrichment_entry, dict):
        return False
    from gate_engine.moneyline.team_acquisition import validate_wnba_ml_hydration
    return validate_wnba_ml_hydration(enrichment_entry).get("hydration_status") == "ACQUIRED"
...
def _canonical_selection_id(
    ...
    *,
    ...
    Hash over (sport, player, prop, side, line_bucket), optionally including
    an event identity for a narrow event-scoped lane.  The optional identity
    keeps the full-board contract byte-for-byte unchanged while preventing a
    same-team doubleheader from collapsing into one scoped moneyline row.
    ...
    if event_identity:
        parts.append(str(event_identity).strip().lower())
    parts = "|".join(parts)
...
def _source_status_requires_coverage_failure(status: Any) -> bool:
    """Return whether an attempted discovery source failed or was partial."""
    value = str(status or "").strip().upper()
    return (
        value.startswith(("FAILED", "ERROR", "UNAVAILABLE", "PARTIAL"))
        or " FAILED" in value
        or " PARTIAL" in value
    )
...
def _source_coverage_failures(
    ...
    """Serialize failed required source attempts for manifest diagnostics."""
    return [
        f"{sport}:source_coverage:{source_key}:{status}"
        for source_key, status in source_status.items()
        if _source_status_requires_coverage_failure(status)
    ]
...
def _apply_source_coverage_reconciliation(
    reconciliation: dict[str, Any],
    source_failures: list[str],
) -> dict[str, Any]:
    """
    Make source completeness part of the canonical reconciliation contract.

    Exact row accounting remains independently visible.  A terminal board is
    never authoritative if an attempted required discovery source was failed
    or partial, because a successful alternate source cannot prove that the
    union was complete.
    """
    row_reconciled = bool(reconciliation.get("reconciled"))
    source_coverage_reconciled = not source_failures
    result = dict(reconciliation)
    result.update({
        "row_reconciled": row_reconciled,
        "source_coverage_reconciled": source_coverage_reconciled,
        "source_coverage_failures": list(source_failures),
        "reconciled": row_reconciled and source_coverage_reconciled,
    })
    return result
...
_PREREQUISITE_FAILURE_VALUES = frozenset({
    "FAILED", "FAIL", "ERROR", "UNAVAILABLE", "MISSING", "INVALID",
    "NOT_READY", "UNSATISFIED", "BLOCKED",
})
...
def _purge_non_pregame_moneyline_cards(
    ...
    """Remove started moneyline rows at the final publication boundary.

    This observes timing only.  It does not assign labels, change a score, or
    introduce a replacement terminal outcome.
    """
    from gate_engine.moneyline.pregame import pregame_exclusion_reason

    from gate_engine.market_family import MarketFamily, classify_market_family

    daily_full_game_aliases = frozenset({"full_game_h2h", "full_game_moneyline"})
    excluded: list[str] = []
    for bucket_name in _TERMINAL_BUCKETS:
        retained: list[dict[str, Any]] = []
        for card in scan_result.get(bucket_name, []):
            market_type = str(card.get("market_type") or "").strip().lower()
            is_moneyline_outcome = (
                classify_market_family(card) == MarketFamily.OUTRIGHT_WINNER
                or market_type in daily_full_game_aliases
            )
            if not is_moneyline_outcome:
                retained.append(card)
                continue
            reason = pregame_exclusion_reason(card)
            if reason is None:
                retained.append(card)
                continue
            excluded.append(str(card.get("canonical_selection_id") or ""))
            logger.info(
                "daily final publication removed non-pregame moneyline row=%s reason=%s",
                card.get("canonical_selection_id"), reason,
            )
        scan_result[bucket_name] = retained
    return excluded
...
def run_daily_orchestration(
    *,
    ...
    scope: str                         = "FULL_BOARD",
    scope_requested_at: str | None     = None,
    ...
    # ---- Immutable scoped intent (Task #277) --------------------------------
    # The scope is read from the persisted run identity only; an unknown scope
    # fails closed rather than silently running the full board.
    from gate_engine.daily_moneyline_scope import (
        SCOPE_FULL_BOARD,
        SCOPE_MONEYLINE_REMAINING_TODAY,
    )
    if scope not in (SCOPE_FULL_BOARD, SCOPE_MONEYLINE_REMAINING_TODAY):
        raise ValueError(f"INVALID_DAILY_SCOPE:{scope}")
    scoped_moneyline = scope == SCOPE_MONEYLINE_REMAINING_TODAY
    if scoped_moneyline and not scope_requested_at:
    ...
    for sport in requested_sports:
        ...
        if scoped_moneyline:
            # Narrow remaining-today moneyline acquisition: the broad prop
            # board is structurally never requested for a scoped run.
            from gate_engine.daily_moneyline_scope import (
                discover_remaining_today_moneyline,
            )
            raw_props, src_status = discover_remaining_today_moneyline(
                sport,
                run_date=run_date,
                run_timezone=run_timezone,
                scope_requested_at=str(scope_requested_at),
            )
        else:
            raw_props, src_status = _union_props_for_sport(sport)
        source_status_all.update(src_status)
        source_failures = _source_coverage_failures(sport, src_status)
        if source_failures:
            failed_modules.extend(source_failures)
            execution_notes.append(
                f"{sport}: SOURCE_COVERAGE_INCOMPLETE "
                f"({len(source_failures)} attempted source failure(s))"
            )
    ...
    if not checkpoint_persisted:
        ...
        if not scoring_started:
        ...
        elif scoped_moneyline:
            # ---- Scoped scoring: existing LLP moneyline lane ONLY ----------
            # The prop pipeline / broad scanner is structurally never invoked
            # for a scoped run; every candidate routes through the same
            # classification + MONEYLINE_V1 validation + specialist scorer as
            # direct OUTRIGHT_WINNER requests.
            from gate_engine.daily_moneyline_scope import (
                score_scoped_moneyline_rows,
            )
            scoped_rows = [
                row for sport in scanned_sports
                for row in props_by_sport.get(sport, [])
            ]
            try:
                scoped_result = score_scoped_moneyline_rows(scoped_rows)
                scan_result = _empty_scan_result()
                for bucket in _TERMINAL_BUCKETS:
                    scan_result[bucket] = list(scoped_result.get(bucket, []))
                scan_result["failed_modules"] = list(
                    scoped_result.get("failed_modules", [])
                )
                execution_notes.extend(scoped_result.get("execution_notes", []))
                execution_notes.append(
                    f"SCOPED_MONEYLINE_SCORED:{len(scoped_rows)}"
                )
            except Exception as exc:
                logger.exception("scoped moneyline scoring failed run=%s", run_id)
                terminal_failure_reasons.append((
                    f"SCOPED_MONEYLINE_SCORING_FAILED:{type(exc).__name__}",
                    "daily_orchestrator.scoped_moneyline",
                ))
                scan_result = _empty_scan_result(run_status="DEGRADED")
                failed_modules.append(
                    f"daily_moneyline_scope:SCORING_FAILED:{type(exc).__name__}"
                )
            ...
            primary_result, primary_failure = _run_scan_isolated(
                ...
                "_source_status_by_sport": {
                    sport: {
                        source_key: status
                        for source_key, status in source_status_all.items()
                        if source_key.startswith(f"{sport}_")
                    }
                    for sport in scanned_sports
                },
                    "_persist_results": False,
    ...
    final_refresh_excluded_ids = _purge_non_pregame_moneyline_cards(scan_result)
    if final_refresh_excluded_ids:
        # The final publication board no longer contains started events.  Keep
        # reconciliation against that publication set rather than manufacture a
        # terminal label for an ineligible live event.
        discovered_ids.difference_update(final_refresh_excluded_ids)
        execution_notes.append(
    ...
    reconciliation = _apply_source_coverage_reconciliation(
        ...
        [
            failure
            for sport in requested_sports
            for failure in _source_coverage_failures(
                sport,
                {
                    source_key: status
                    for source_key, status in source_status_all.items()
        ...
        try:
            ...
            if not finalize_run(
                ...
                orchestration_metadata={
                    ...
                    "run_timezone": run_timezone,
                    "final_pregame_excluded_ids": final_refresh_excluded_ids,
                },
                execution_owner=execution_owner,
            ):
