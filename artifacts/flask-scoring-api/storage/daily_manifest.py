...
CREATE TABLE IF NOT EXISTS wow_daily_runs (
    ...
    request_scope       TEXT        NOT NULL DEFAULT 'FULL_BOARD',
    scope_requested_at  TIMESTAMPTZ,
                    ...
                    cur.execute(
                        ...
                        ALTER TABLE wow_daily_runs
                            ...
                            ADD COLUMN IF NOT EXISTS request_scope TEXT
                                NOT NULL DEFAULT 'FULL_BOARD',
                            ADD COLUMN IF NOT EXISTS scope_requested_at TIMESTAMPTZ,
...
def create_or_get_run(
    *,
    ...
    request_scope: str = "FULL_BOARD",
    scope_requested_at: str | None = None,
) -> tuple[dict[str, Any] | None, bool]:
    """Atomically create or reuse one canonical run identity.

    Scope immutability: the stored ``request_scope`` is part of the durable
    request identity.  Reusing an idempotency key with a different scope is a
    hard conflict (``IDEMPOTENCY_KEY_SCOPE_MISMATCH``) — an existing run's
    scope is never mutated by a retry.
    """
        ...
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    INSERT INTO wow_daily_runs (
                        ...
                        requested_sports, session_id, runtime_provenance,
                         ...
                         deadline_at, request_scope, scope_requested_at,
                        run_status, progress_stage, progress_updated_at
                    ) VALUES (
                        %s, %s, %s, %s, %s, %s, %s,
                        %s, %s, %s, %s, %s, %s, 'ACCEPTED', 'ACCEPTED', NOW()
                    )
                ...
                if result is not None and idempotency_key:
                    ...
                    existing_scope = str(
                        result.get("request_scope") or "FULL_BOARD"
                    )
            ...
            if run_date:
                cur.execute(
                    ...
                    SELECT run_id, run_date, started_at, finished_at,
                           environment, run_status, progress_stage,
                           progress_detail, total_discovered,
                           requested_sports, scanned_sports, missing_sports
                    FROM wow_daily_runs
                    WHERE run_date = %s
