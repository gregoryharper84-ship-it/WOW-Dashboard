...
**Status:** Step 4 smoke-tested (8/8 PASS). Character-safety trim applied (7,589 chars), then Greg (ChatGPT) returned a HOLD verdict on that trimmed block citing 8 backend-vocabulary/logic-consistency blockers. Step 6 fixed those blockers against live backend source (grew to 8,667 chars, over limit). Step 7 trimmed the corrected block back down to 7,685 chars. Step 8: Greg returned **APPROVED FOR STAGING after one required micro-edit** on the WNBA triple-risk line (reworded to GPT-advisory only / not backend-enforced). Step 9 (this pass): an external "alignment request" relayed a mismatched 7,038-char/`GPT_ADVISORY_BUCKET`/`N/T`-worded version that did not match the repo file on disk — flagged, not applied. External Claude then independently confirmed (against raw source excerpts pasted verbatim into the review thread) that `SOURCE_CONFLICT` has two distinct backend paths — a hard classifier-terminal path and a softer source-grade-ceiling path — which the block's prior unified wording did not capture. Fixed: SOURCE STATUS now spells out both paths explicitly; SOURCE GRADES cross-references it instead of repeating the old unified claim. Also corrected `N-T` → the literal backend token `N/T`. The two-path fix initially pushed the block to 8,040 chars (over limit); trimmed non-load-bearing wording (one MARKET BUCKETS example, CROSS-MARKET phrasing, CAPS phrasing) with zero rule/label/threshold/gate-name loss to land at **7,994 characters (8,032 bytes)** — under the 8,000-char limit. All 12 confirmed `PropLabel` enum members still present verbatim in TERMINAL BUCKETS. No backend code was read or touched this pass. Do NOT paste into the live Custom GPT config yet. Do NOT mark Deployed until the user confirms the text has actually been pasted into the live Custom GPT builder, per the same process used for the LLP GPT patch.
...
## Corrected instructions block (Step 9: SOURCE_CONFLICT two-path fix + N/T token correction — 7,994 chars, under the 8,000-char limit — pending final review before staging/deployment — DEPLOYMENT HOLD)
...
SOURCE GRADES
A/A- = direct API/official gamelog, full trust, no cap. B = trusted stat site, caps at MODEL_QUALIFIED_HOLD unless corroborated. C/D/N/T (no timestamp) cap at RESEARCH_INTEREST, cannot raise model_prob/edge_vs_friction/market_edge_confirmed. SOURCE_CONFLICT here = source-grade path (see SOURCE STATUS): ceiling only, not automatic terminal. Stale critical source = rerun.
