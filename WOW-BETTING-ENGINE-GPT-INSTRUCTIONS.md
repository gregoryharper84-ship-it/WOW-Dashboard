...
**Status:** Step 4 smoke-tested (8/8 PASS). Character-safety trim applied (7,589 chars), then Greg (ChatGPT) returned a HOLD verdict on that trimmed block citing 8 backend-vocabulary/logic-consistency blockers. Step 6 fixed those blockers against live backend source (grew to 8,667 chars, over limit). Step 7 trimmed the corrected block back down to 7,685 chars. Step 8 (this pass): Greg returned **APPROVED FOR STAGING after one required micro-edit** — the WNBA triple-risk line used "caps at," which made a GPT-only advisory rule sound backend-enforced; reworded to explicitly say GPT-advisory only / not backend-enforced / flag in advisory_notes / do not cap or kill without independent backend support. Block is now **7,753 characters (7,793 bytes)** — still comfortably under the 8,000-char Custom GPT limit. The proprietary/no-market/WNBA-advisory smoke test was rerun against the edited block: PASS. Do NOT paste into the live Custom GPT config yet. Do NOT mark Deployed until the user confirms the text has actually been pasted into the live Custom GPT builder, per the same process used for the LLP GPT patch.
...
## Corrected instructions block (APPROVED FOR STAGING after WNBA micro-edit — 7,753 chars, under the 8,000-char limit — DEPLOYMENT HOLD until pasted into the live Custom GPT config and confirmed)
