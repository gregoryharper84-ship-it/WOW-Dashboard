...
0. **WOW-PATCH-2026-07-05-WOW-GPT-RECONCILE** — do not lose. Status: **Step 7 complete (this session): corrected block trimmed from 8,667 → 7,685 chars (under the 7,700 target), all 8 Step 6 backend-vocabulary corrections verified intact, 8-test smoke-test spec rerun 8/8 PASS.** Remaining before Deployed: Greg/ChatGPT (and the user) must eyeball the exact final block in `WOW-BETTING-ENGINE-GPT-INSTRUCTIONS.md` and confirm, then it can be pasted into the live Custom GPT config. Do NOT re-trim further without re-verifying the two previously-overturned Greg claims (SOURCE_CONFLICT, NO_PLAY as real row-terminal buckets) are still intact — see Step 6/7 entries above. Remaining real backend gaps from the earlier pass (no props-side MARKET BUCKETS system, no `REJECT_ROLE_STATUS`/`REJECT_LINE_VALUE`/`REJECT_CONTEXT` labels, no WNBA triple-risk kill rule) still need a user decision on whether to (a) leave the GPT instructions matching backend reality as-is, or (b) file a follow-up patch to build the missing backend mechanisms. See `WOW-BETTING-ENGINE-GPT-INSTRUCTIONS.md` for full detail.
