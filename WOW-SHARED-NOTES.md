...
0. **WOW-PATCH-2026-07-05-WOW-GPT-RECONCILE** — do not lose. Status: **APPROVED FOR STAGING (Step 8, this session) after a required WNBA triple-risk wording micro-edit; block is now 7,753 chars, under the 8,000-char limit. DEPLOYMENT: HOLD.** Only remaining step: user pastes the exact block from `WOW-BETTING-ENGINE-GPT-INSTRUCTIONS.md` into the live Custom GPT config and confirms it's live — only then update status to Deployed. Do NOT re-edit the block further without user/Greg sign-off first. Do NOT re-trim without re-verifying the two previously-overturned Greg claims (SOURCE_CONFLICT, NO_PLAY as real row-terminal buckets) are still intact — see Step 6/7/8 entries above. Remaining real backend gaps from the earlier pass (no props-side MARKET BUCKETS system, no `REJECT_ROLE_STATUS`/`REJECT_LINE_VALUE`/`REJECT_CONTEXT` labels, no WNBA triple-risk kill rule in backend code) still need a user decision on whether to (a) leave the GPT instructions matching backend reality as-is, or (b) file a follow-up patch to build the missing backend mechanisms. See `WOW-BETTING-ENGINE-GPT-INSTRUCTIONS.md` for full detail.
